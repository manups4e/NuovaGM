	...
	internal static class RuotaPanoramica
	{
		...
		private static async Task MuoviRuota()
		{
			if (!Ruota.Ferma && Ruota.Entity != null)
			{
				...
				if (IsAudioSceneActive("FAIRGROUND_RIDES_FERRIS_WHALE")) SetAudioSceneVariable("FAIRGROUND_RIDES_FERRIS_WHALE", "HEIGHT", SessionCache.Cache.MyPlayer.Ped.Position.Z - 13f);
					...
					if (Math.Abs(Ruota.Rotazione - Cabine[i].Gradient) < 0.05f)
					{
						...
						switch (Ruota.State)
						{
							case "FACCIO_SALIRE":
								...
								BaseScript.TriggerServerEvent("lprp:ruotapanoramica:playerSale", SessionCache.Cache.MyPlayer.Ped.NetworkId, Ruota.Gradient);

								break;
							case "FACCIO_SCENDERE":
								BaseScript.TriggerServerEvent("lprp:ruotapanoramica:playerScende", SessionCache.Cache.MyPlayer.Ped.NetworkId, Ruota.Gradient);
		...
		private static async void playerSale(int player, int cabina)
		{
			...
			{
				if (Personaggio.NetworkId != SessionCache.Cache.MyPlayer.Ped.NetworkId)
				...
				Vector3 attCoords = GetOffsetFromEntityGivenWorldCoords(Cabina.Entity.Handle, SessionCache.Cache.MyPlayer.Ped.Position.X, SessionCache.Cache.MyPlayer.Ped.Position.Y, SessionCache.Cache.MyPlayer.Ped.Position.Z);
				AttachEntityToEntity(Personaggio.Handle, Cabina.Entity.Handle, 0, attCoords.X, attCoords.Y, attCoords.Z, 0f, 0f, SessionCache.Cache.MyPlayer.Ped.Heading, false, false, false, false, 2, true);
		...
		private static async void playerScende(int player, int cabina)
		{
			...
			if (Personaggio == SessionCache.Cache.MyPlayer.Ped)
			{
		...
		{
			if (SessionCache.Cache.MyPlayer.Ped.IsInRangeOf(new Vector3(-1661.95f, -1127.011f, 12.6973f), 20f))
			{
				if (!IsAudioSceneActive("FAIRGROUND_RIDES_FERRIS_WHALE")) StartAudioScene("FAIRGROUND_RIDES_FERRIS_WHALE");

				if (SessionCache.Cache.MyPlayer.Ped.IsInRangeOf(new Vector3(-1661.95f, -1127.011f, 12.6973f), 1.375f))
				{
		...
		private static async void func_105(RuotaPanoramicaCamGamePad uParam0)
		{
			...
			if (uParam0.Valore0)
			{
				...
				if (DoesCamExist(uParam0.CamEntity.Handle))
				{
					...
					{
						SetCamRot(uParam0.CamEntity.Handle, (SessionCache.Cache.MyPlayer.Ped.Rotation + new Vector3(uParam0.Valore11, 0f, uParam0.Valore12)).X, (SessionCache.Cache.MyPlayer.Ped.Rotation + new Vector3(uParam0.Valore11, 0f, uParam0.Valore12)).Y, (SessionCache.Cache.MyPlayer.Ped.Rotation + new Vector3(uParam0.Valore11, 0f, uParam0.Valore12)).Z, 2);
					}
		...
		private static async void func_101(RuotaPanoramicaCamTastiera uParam0, bool bParam1, bool bParam2, bool bParam3, bool bParam4, float fParam5, bool bParam6, float fParam7, bool bParam8)
		{
			...
			uParam0.CamEntity.Rotation = SessionCache.Cache.MyPlayer.Ped.Rotation + Cam2Tastiera.Valore14;
		}
