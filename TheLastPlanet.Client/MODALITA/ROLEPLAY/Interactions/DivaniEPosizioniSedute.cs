...
namespace TheLastPlanet.Client.RolePlay.Interactions
{
	internal static class DivaniEPosizioniSedute
	{
		...
		{
			SediaClosest = World.GetAllProps().Where(o => Sedie.Contains((ObjectHash)(uint)o.Model.Hash)).FirstOrDefault(o => Cache.PlayerCache.MyPlayer.User.Posizione.Distance(o.Position) < 1.375f);
					...
					if (!Seduto)
					{
						...
						if (Vector3.Distance(SediaClosest.Position, ped.Position) < 0.35f)
							if (!IsPedActiveInScenario(ped.Handle))
								if (!p.IsDead && !(p.Health < 1) && !Cache.PlayerCache.MyPlayer.User.StatiPlayer.RolePlayStates.InVeicolo)
								{
