...
namespace TheLastPlanet.Client.RolePlay.Lavori.Generici.Pescatore
{
	...
	internal static class PescatoreClient
	{
		...
		public static async Task ControlloPesca()
		{
			...
			if (Main.spawned)
			{
				if (PerVendereIlPesce.Any(o => Cache.PlayerCache.MyPlayer.User.GetInventoryItem(o).Item1))
				{
					if (!mostrablip)
					{
				...
				/*			if (Vector3.Distance(Cache.Char.posizione.ToVector3(), PuntiPesca.AffittoBarca) < 2f && !Cache.Char.StatiPlayer.RolePlayStates.InVeicolo)
							{
