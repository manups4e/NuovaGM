...
namespace TheLastPlanet.Client.MODALITA.ROLEPLAY.Lavori.Generici.Cacciatore
{
    internal static class CacciatoreClient
    {
        ...
        private static async void ControlloAnimale(int victim, int player, uint weaponHash, bool wasMeleeDamage)
        {
            ...
            if (assassino.Character != Cache.PlayerCache.MyPlayer.Ped) return;
        ...
        private static async void ApriMenuAffittoArmi()
        {
            ...
            if (!StaCacciando)
            {
                ...
                {
                    if (Cache.PlayerCache.MyPlayer.User.HasLicense("Caccia"))
                    {
                        if (item == armi)
                        {
                            ...
                            if (!Cache.PlayerCache.MyPlayer.User.HasWeapon(DaFuoco))
                            ...
                            if (!Cache.PlayerCache.MyPlayer.User.HasWeapon(Bianca))
                        ...
                        else if (item == inizia)
                        {
                            ...
                            foreach (string s in animalGroups) Cache.PlayerCache.MyPlayer.Ped.RelationshipGroup.SetRelationshipBetweenGroups(new RelationshipGroup(Funzioni.HashInt(s)), Relationship.Dislike, true);
