	...
	internal static class Manager
	{
				...
				if (Cache.PlayerCache.MyPlayer.User.StatiPlayer.InVeicolo)
				{
					...
					if (playerPed.CurrentVehicle.PassengerCount > 0)
						foreach (Ped p in playerPed.CurrentVehicle.Passengers)
						{
							...
							pl.GetPlayerData().StatiPlayer.Istanza.Istanzia(Cache.PlayerCache.MyPlayer.Player.ServerId, Cache.PlayerCache.MyPlayer.User.StatiPlayer.Istanza.Instance);
					...
					foreach (OwnedVehicle veh in Cache.PlayerCache.MyPlayer.User.CurrentChar.Veicoli.Where(veh => veh.Garage.Garage == Cache.PlayerCache.MyPlayer.User.StatiPlayer.Istanza.Instance).Where(veh => veh.Garage.InGarage))
					{
		...
		public static async Task MarkerDentro()
		{
			...
			if (Cache.PlayerCache.MyPlayer.User.StatiPlayer.Istanza.Stanziato)
				if (Proprietà.Appartamenti.ContainsKey(Cache.PlayerCache.MyPlayer.User.StatiPlayer.Istanza.Instance))
				{
					ConfigCase app = Proprietà.Appartamenti[Cache.PlayerCache.MyPlayer.User.StatiPlayer.Istanza.Instance];
