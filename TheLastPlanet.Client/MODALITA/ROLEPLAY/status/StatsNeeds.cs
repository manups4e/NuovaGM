...
namespace TheLastPlanet.Client.RolePlay.Core.Status
{
	...
	internal static class StatsNeeds
	{
		...
		private static async void Clacson()
		{
			...
			if (Cache.PlayerCache.MyPlayer.User.Status.RolePlayStates.InVeicolo) Client.Instance.AddTick(Horn);
			await BaseScript.Delay(30000);
			p.CancelRagdoll();
			if (Cache.PlayerCache.MyPlayer.User.Status.RolePlayStates.InVeicolo) Client.Instance.RemoveTick(Horn);
		}
			...
			if (stanchezza.Val >= 80.0f)
				if (Game.GameTime - _svieniTimer > 600000)
				{
					if (Funzioni.GetRandomInt(100) > 85)
					{
						...
						else if (Cache.PlayerCache.MyPlayer.User.Status.RolePlayStates.InVeicolo || playerPed.IsInFlyingVehicle)
						{
		...
		public static void Stamina(Ped playerPed, Player me, Statistica stam)
		{
			...
			stam.ChangeVal = !Cache.PlayerCache.MyPlayer.User.Status.RolePlayStates.InVeicolo ? playerPed.IsSprinting || playerPed.IsSwimmingUnderWater ? 0.002f : playerPed.IsRunning || playerPed.IsSwimming ? 0.001f : 0f : playerPed.CurrentVehicle.Model.IsBicycle ? 0.003f : 0f;
		...
		public static void Driving(Ped playerPed, Player me, Statistica driving)
		{
			...
			if (Cache.PlayerCache.MyPlayer.User.Status.RolePlayStates.InVeicolo && playerPed.SeatIndex == VehicleSeat.Driver)
			{
