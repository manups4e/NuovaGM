...
namespace TheLastPlanet.Client.Negozi
{
	internal static class BarberClient
	{
		...
		{
			while (!CachePlayer.Cache.MyPlayer.Ped.IsInRangeOf(S, 1f)) await BaseScript.Delay(100);

			if (CachePlayer.Cache.MyPlayer.Ped.IsInRangeOf(S, 1f) && IsPedUsingScenario(PlayerPedId(), "PROP_HUMAN_SEAT_CHAIR_MP_PLAYER"))
			{
				HUD.ShowHelp("Ricorda che puoi anche usare il ~b~MOUSE~w~ per selezionare i colori e l'opacità.");
				ShowCam(S, Ch, C);
				BarberMenu(CachePlayer.Cache.MyPlayer.Character.CurrentChar.skin.sex == "Maschio" ? Client.Impostazioni.Negozi.Barbieri.Maschio : Client.Impostazioni.Negozi.Barbieri.Femmina, Menu);
			}
		}
		...
		public static async void ShowCam(Vector3 C, float Ch, Vector3 O)
		{
			...
			Camm.PointAt(CachePlayer.Cache.MyPlayer.Ped.Bones[Bone.SKEL_Head].Position);
		...
		private static async void BarberMenu(BarbieriTesta Tipo, string NomeNegozio)
		{
			...
			Skin skin = CachePlayer.Cache.MyPlayer.Character.CurrentChar.skin;
			...
			MenuPrincipale.OnListSelect += async (menu, _listItem, itemindex) =>
			{
				if (_listItem == Capelli)
				{
					...
					{
						if (CachePlayer.Cache.MyPlayer.Character.Money >= obj.price)
						{
						...
						{
							if (CachePlayer.Cache.MyPlayer.Character.Bank >= obj.price)
							{
								skin.hair.style = obj.var;
								skin.hair.color[0] = capCol1.CurrentSelection;
								skin.hair.color[1] = capCol2.CurrentSelection;
