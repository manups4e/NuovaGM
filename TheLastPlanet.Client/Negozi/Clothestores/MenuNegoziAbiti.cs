...
namespace TheLastPlanet.Client.Negozi
{
	internal static class MenuNegoziAbiti
	{
		...
		private static async void StartAnim(string lib, string anim)
		{
			...
			await CachePlayer.Cache.MyPlayer.Ped.Task.PlayAnimation(lib, anim, 8f, -8f, -1, AnimationFlags.Loop, 0);
		}

		private static async void StartAnimN(string lib, string anim)
		{
			CachePlayer.Cache.MyPlayer.Ped.BlockPermanentEvents = true;
			await CachePlayer.Cache.MyPlayer.Ped.Task.PlayAnimation(lib, anim, 4.0f, -2.0f, -1, AnimationFlags.None, 0);
		}

		private static void StartScenario(string anim)
		{
			CachePlayer.Cache.MyPlayer.Ped.Task.StartScenario(anim, CachePlayer.Cache.MyPlayer.Character.posizione.ToVector3());
		}

		public static async Task UpdateDress(dynamic dress)
		{
			int id = CachePlayer.Cache.MyPlayer.Ped.Handle;
		...
		public static async void MenuVest(List<Completo> Completi, string anim, string nome)
		{
			...
			CachePlayer.Cache.MyPlayer.Ped.BlockPermanentEvents = true;
			SetPedAlternateMovementAnim(CachePlayer.Cache.MyPlayer.Ped.Handle, 0, anim, "try_shirt_base", 4.0f, true);
			await CachePlayer.Cache.MyPlayer.Ped.Task.PlayAnimation(anim, "try_shirt_base", 8f, -8f, -1, AnimationFlags.Loop, 0);
			...
			foreach (Completo t in completi)
			{
				...
				if (CachePlayer.Cache.MyPlayer.Character.Money >= t.Price)
				{
					vest.SetRightLabel("~g~$" + t.Price);
				}
				else
				{
					if (CachePlayer.Cache.MyPlayer.Character.Bank >= t.Price)
						vest.SetRightLabel("~g~$" + t.Price);
					else
						vest.SetRightLabel("~r~$" + t.Price);
				}

				if (t.Name != CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Name) continue;
			...
			MenuVest.OnIndexChange += async (sender, index) =>
			{
				...
				await CachePlayer.Cache.MyPlayer.Ped.Task.PlayAnimation(anim, random, 4f, -2f, -1, AnimationFlags.None, 0);
			};
			MenuVest.OnItemSelect += async (_menu, _item, _index) =>
			{
				if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Name == completi[_index].Name && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Description == completi[_index].Description)
				{
					HUD.ShowNotification("Possiedi già quest'abito!", true);
				}
				else
				{
					if (CachePlayer.Cache.MyPlayer.Character.Money >= completi[_index].Price)
					{
						...
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing = new Dressing(completi[_index].Name, completi[_index].Description, completi[_index].ComponentDrawables, completi[_index].ComponentTextures, completi[_index].PropIndices, completi[_index].PropTextures);
					...
					{
						if (CachePlayer.Cache.MyPlayer.Character.Bank >= completi[_index].Price)
						{
			...
			MenuVest.OnMenuStateChanged += async (oldmenu, _menu, state) =>
			{
				...
				await UpdateDress(CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing);
		...
		public static async void MenuPant(List<Singolo> Completi, string anim, string nome)
		{
			...
			CachePlayer.Cache.MyPlayer.Ped.BlockPermanentEvents = true;
			SetPedAlternateMovementAnim(CachePlayer.Cache.MyPlayer.Ped.Handle, 0, anim, "try_trousers_base", 4.0f, true);
			await CachePlayer.Cache.MyPlayer.Ped.Task.PlayAnimation(anim, "try_trousers_base", 8f, -8f, -1, AnimationFlags.Loop, 0);
			...
			foreach (Singolo v in completi)
			{
				...
				foreach (int texture in v.Text)
				{
					...
					if (CachePlayer.Cache.MyPlayer.Character.Money >= money)
					{
						pant.SetRightLabel("~g~$" + money);
					}
					else
					{
						if (CachePlayer.Cache.MyPlayer.Character.Bank >= money)
							pant.SetRightLabel("~g~$" + money);
						else
							pant.SetRightLabel("~r~$" + money);
					}

					if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentDrawables.Pantaloni != v.Modello || CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentTextures.Pantaloni != texture) continue;
				...
				{
					if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentDrawables.Pantaloni == mod && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentTextures.Pantaloni == text)
					{
					...
					if (CachePlayer.Cache.MyPlayer.Character.Money >= val)
					{
						BaseScript.TriggerServerEvent("lprp:abiti:compra", val, 1);
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentDrawables.Pantaloni = mod;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentTextures.Pantaloni = text;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Name = null;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Description = null;
						BaseScript.TriggerServerEvent("lprp:updateCurChar", "chardressing", CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.SerializeToJson());
						ciao.SetRightBadge(BadgeStyle.None);
					...
					{
						if (CachePlayer.Cache.MyPlayer.Character.Bank >= val)
						{
		...
		public static async void MenuScarpe(List<Singolo> Completi, string anim, string nome)
		{
			...
			CachePlayer.Cache.MyPlayer.Ped.BlockPermanentEvents = true;
			SetPedAlternateMovementAnim(CachePlayer.Cache.MyPlayer.Ped.Handle, 0, anim, "try_shoes_base", 4.0f, true);
			await CachePlayer.Cache.MyPlayer.Ped.Task.PlayAnimation(anim, "try_shoes_base", 8f, -8f, -1, AnimationFlags.Loop, 0);
			List<Singolo> completi = Completi.OrderBy(x => x.Price).ToList();
			int money = 0;
			int mod = 0;
			int text = 0;
			...
			foreach (Singolo v in completi)
			{
				...
				foreach (int texture in v.Text)
				{
					...
					if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentDrawables.Scarpe != v.Modello || CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentTextures.Scarpe != texture) continue;
				...
				{
					if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentDrawables.Scarpe == mod && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentTextures.Scarpe == text)
					{
					...
					{
						BaseScript.TriggerServerEvent("lprp:abiti:compra", val, 1);
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentDrawables.Scarpe = mod;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentTextures.Scarpe = text;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Name = null;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Description = null;
						BaseScript.TriggerServerEvent("lprp:updateCurChar", "chardressing", CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.SerializeToJson());
						ciao.SetRightBadge(BadgeStyle.None);
		...
		public static async void MenuOcchiali(List<Singolo> Completi, string anim, string nome)
		{
			...
			CachePlayer.Cache.MyPlayer.Ped.BlockPermanentEvents = true;
			SetPedAlternateMovementAnim(CachePlayer.Cache.MyPlayer.Ped.Handle, 0, anim, "Try_Glasses_Base", 4.0f, true);
			await CachePlayer.Cache.MyPlayer.Ped.Task.PlayAnimation(anim, "Try_Glasses_Base", 8f, -8f, -1, AnimationFlags.Loop, 0);
			List<Singolo> completi = Completi.OrderBy(x => x.Price).ToList();
			int money = 0;
			int mod = 0;
			int text = 0;
			...
			foreach (Singolo v in completi)
			{
				...
				foreach (int texture in v.Text)
				{
					...
					if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie != v.Modello || CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Orecchie != texture) continue;
					Scarp.ParentItem.SetRightBadge(BadgeStyle.Clothes);
					ciao = Scarp.ParentItem;
					pant.SetRightBadge(BadgeStyle.Clothes); // cambiare con la collezione di abiti
					ciaone = pant;
				...
				Scarp.OnItemSelect += async (_menu, _item, _index) =>
				{
					if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie == mod && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Orecchie == text)
					{
					...
					{
						BaseScript.TriggerServerEvent("lprp:abiti:compra", val, 1);
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie = mod;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Orecchie = text;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Name = null;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Description = null;
						BaseScript.TriggerServerEvent("lprp:updateCurChar", "chardressing", CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.SerializeToJson());
						ciao.SetRightBadge(BadgeStyle.None);
		...
		public static async void MenuAccessori(Accessori Accessorio, string anim, string nome)
		{
			...
			await CachePlayer.Cache.MyPlayer.Ped.Task.PlayAnimation(anim, "try_shirt_base", 8f, -8f, -1, AnimationFlags.Loop, 0);
			int money = 0;
			int IntorecAtt = CachePlayer.Cache.MyPlayer.Character.CurrentChar.skin.ears.style;
			int IntcoloOreccAtt = CachePlayer.Cache.MyPlayer.Character.CurrentChar.skin.ears.color;
			...
			foreach (Singolo borsa in Accessorio.Borse)
			{
				...
				if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentDrawables.Borsa_Paracadute != borsa.Modello) continue;
			...
			{
				if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentDrawables.Borsa_Paracadute == IntBors)
				{
				...
				{
					BaseScript.TriggerServerEvent("lprp:abiti:compra", val, 1);
					CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentDrawables.Borsa_Paracadute = IntBors;
					CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.ComponentTextures.Borsa_Paracadute = 0;
					CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Name = null;
					CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Description = null;
					BaseScript.TriggerServerEvent("lprp:updateCurChar", "chardressing", CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.SerializeToJson());
			...
			foreach (Singolo cappellino in Accessorio.Testa.Cappellini)
			{
				...
				if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Cappelli_Maschere == -1)
				{
					CappRim.SetRightBadge(BadgeStyle.Clothes);
					CappAtt = CappRim;
				}

				if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Cappelli_Maschere != -1 && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Cappelli_Maschere == cappellino.Modello)
				{
				...
				foreach (int texture in cappellino.Text)
				{
					...
					if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Cappelli_Maschere != cappellino.Modello || CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Cappelli_Maschere != texture) continue;
				...
				{
					if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Cappelli_Maschere == IntCappAtt && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Cappelli_Maschere == IntCappAttMod)
					{
					...
					else
					{
						...
						{
							BaseScript.TriggerServerEvent("lprp:abiti:compra", val, 1);
							CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Cappelli_Maschere = IntCappAtt;
							CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Cappelli_Maschere = IntCappAttMod;
							CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Name = null;
							CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Description = null;
							BaseScript.TriggerServerEvent("lprp:updateCurChar", "chardressing", CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.SerializeToJson());
			...
			Capp.OnItemSelect += (_menu, _item, _index) =>
			{
				...
				else
				{
					...
					{
						BaseScript.TriggerServerEvent("lprp:abiti:compra", val, 1);
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Cappelli_Maschere = -1;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Cappelli_Maschere = -1;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Name = null;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Description = null;
						BaseScript.TriggerServerEvent("lprp:updateCurChar", "chardressing", CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.SerializeToJson());
						CappAtt.SetRightBadge(BadgeStyle.None);
			...
			Orecc.OnListChange += (_menu, _listItem, _newIndex) =>
			{
				...
				if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie == Accessorio.Testa.Orecchini.OrderBy(x => x.Price).ToList()[_newIndex].Modello)
				{
			...
			{
				if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie == Accessorio.Testa.Orecchini.OrderBy(x => x.Price).ToList()[_listIndex].Modello && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie == -1)
				{
					HUD.ShowNotification("Non puoi rimuovere 2 volte gli orecchini!!", true);
				}
				else if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie == Accessorio.Testa.Orecchini.OrderBy(x => x.Price).ToList()[_listIndex].Modello && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie != -1)
				{
				...
				else
				{
					...
					{
						BaseScript.TriggerServerEvent("lprp:abiti:compra", val, 1);
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie = Accessorio.Testa.Orecchini.OrderBy(x => x.Price).ToList()[_listIndex].Modello;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Orecchie = 0;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.skin.ears.style = Accessorio.Testa.Orecchini.OrderBy(x => x.Price).ToList()[_listIndex].Modello;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.skin.ears.color = 0;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Name = null;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Description = null;
						BaseScript.TriggerServerEvent("lprp:updateCurChar", "chardressing", CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.SerializeToJson());
						HUD.ShowNotification("Hai speso ~g~" + val + "$~w~, in contanti");
			...
			foreach (Singolo aurico in Accessorio.Testa.Auricolari.OrderBy(x => x.Price).ToList())
			{
				...
				if (CachePlayer.Cache.MyPlayer.Character.Money >= aurico.Price)
				{
					auricolari.SetRightLabel("~g~$" + aurico.Price);
				}
				else
				{
					if (CachePlayer.Cache.MyPlayer.Character.Bank >= aurico.Price)
						auricolari.SetRightLabel("~g~$" + aurico.Price);
					else
			...
			Orecc.OnItemSelect += async (_menu, _item, _index) =>
			{
				...
				if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie == Accessorio.Testa.Auricolari.OrderBy(x => x.Price).ToList()[_index - 1].Modello && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie == -1)
				{
					HUD.ShowNotification("Non puoi rimuovere 2 volte l'auricolare!!", true);
				}
				else if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie == Accessorio.Testa.Auricolari.OrderBy(x => x.Price).ToList()[_index - 1].Modello && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie != -1)
				{
				...
				else
				{
					...
					{
						BaseScript.TriggerServerEvent("lprp:abiti:compra", val, 1);
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orecchie = Accessorio.Testa.Auricolari.OrderBy(x => x.Price).ToList()[_index - 1].Modello;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Orecchie = 0;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.skin.ears.style = Accessorio.Testa.Auricolari.OrderBy(x => x.Price).ToList()[_index - 1].Modello;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.skin.ears.color = 0;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Name = null;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Description = null;
						BaseScript.TriggerServerEvent("lprp:updateCurChar", "chardressing", CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.SerializeToJson());
			...
			foreach (Singolo orologio in Accessorio.Orologi)
			{
				...
				if (orologio.Modello == -1)
				{
					...
					if (CachePlayer.Cache.MyPlayer.Character.Money >= orologio.Price)
					{
						NoOrol.SetRightLabel("~g~$" + orologio.Price);
					}
					else
					{
						if (CachePlayer.Cache.MyPlayer.Character.Bank >= orologio.Price)
							NoOrol.SetRightLabel("~g~$" + orologio.Price);
						else
				...
				foreach (int v in orologio.Text)
				{
					...
					if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orologi == orologio.Modello && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Orologi == v)
					{
						if (orologio.Modello == -1)
						{
				...
				{
					if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orologi == IntOrolAtt && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Orologi == IntOrolMod)
					{
					...
					else
					{
						...
						{
							BaseScript.TriggerServerEvent("lprp:abiti:compra", val, 1);
							CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orologi = IntOrolAtt;
							CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Orologi = IntOrolMod;
							CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Description = null;
			...
			Orol.OnItemSelect += async (_menu, _item, _index) =>
			{
				...
				if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orologi == -1 && IntOrolAtt == -1)
				{
				...
				else
				{
					...
					{
						BaseScript.TriggerServerEvent("lprp:abiti:compra", val, 1);
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Orologi = -1;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Orologi = -1;
						OrolAtt.SetRightBadge(BadgeStyle.None);
						OrolAtt = _menu.MenuItems[_menu.CurrentSelection];
						OrolAtt.SetRightBadge(BadgeStyle.Clothes);
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.skin.ears.style = Accessorio.Orologi.OrderBy(x => x.Price).ToList()[_index].Modello;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Description = null;
						BaseScript.TriggerServerEvent("lprp:updateCurChar", "chardressing", CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.SerializeToJson());
						if (val > 0) HUD.ShowNotification("Hai speso ~g~" + val + "$~w~, in contanti");
					}
			...
			{
				if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Bracciali == Accessorio.Bracciali.OrderBy(x => x.Price).ToList()[_listIndex].Modello && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Bracciali == -1)
				{
					HUD.ShowNotification("Non puoi rimuovere 2 volte un bracciale!!", true);
				}
				else if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Bracciali == Accessorio.Bracciali.OrderBy(x => x.Price).ToList()[_listIndex].Modello && CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Bracciali != -1)
				{
				...
				else
				{
					...
					{
						BaseScript.TriggerServerEvent("lprp:abiti:compra", val, 1);
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropIndices.Bracciali = Accessorio.Bracciali.OrderBy(x => x.Price).ToList()[_listIndex].Modello;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.PropTextures.Bracciali = 0;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Name = null;
						CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.Description = null;
						BaseScript.TriggerServerEvent("lprp:updateCurChar", "chardressing", CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing.SerializeToJson());
						HUD.ShowNotification("Hai speso ~g~" + val + "$~w~, in contanti");
			...
			HUD.MenuPool.OnMenuStateChanged += async (oldmenu, newmenu, state) =>
			{
				...
				else if (state == MenuState.ChangeForward)
				{
					...
					{
						float newheading = CachePlayer.Cache.MyPlayer.Ped.Heading - 180f;
						CachePlayer.Cache.MyPlayer.Ped.Task.AchieveHeading(newheading, 1000);
						PointCamAtPedBone(NegozioAbitiClient.camm.Handle, PlayerPedId(), 24818, 0.0f, 0.0f, 0.0f, true);
					}
					else if (newmenu == Orecc)
					{
						...
						CachePlayer.Cache.MyPlayer.Ped.Task.LookAt(new Vector3(NegozioAbitiClient.camm.Position.X + 5f, NegozioAbitiClient.camm.Position.Y, NegozioAbitiClient.camm.Position.Z));
					}
				...
				else if (state == MenuState.ChangeBackward)
				{
					...
					else if (oldmenu == Orecc)
					{
						...
						await UpdateDress(CachePlayer.Cache.MyPlayer.Character.CurrentChar.dressing);
						CachePlayer.Cache.MyPlayer.Ped.Task.LookAt(NegozioAbitiClient.camm.Position);
					}
