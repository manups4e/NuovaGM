...
namespace TheLastPlanet.Client.Veicoli
{
	public static class FuelClient
	{
		...
		public static async void spawnTanker(Tanker spot, string plate)
		{
			...
			SessionCache.Cache.MyPlayer.Ped.SetIntoVehicle(jobTruck, VehicleSeat.Driver);
		...
		public static void FillFuel()
		{
			...
			{
				SetVehicleFuelLevel(SessionCache.Cache.MyPlayer.Ped.CurrentVehicle, ClientSession.Impostazioni.Veicoli.DanniVeicoli.FuelCapacity);
		...
		public static void FuelLevel(float level)
		{
			...
			SetVehicleFuelLevel(SessionCache.Cache.MyPlayer.Ped.CurrentVehicle, level);
		...
		public static void SAddFuel(int amount)
		{
			...
			{
				float dist = Vector3.Distance(SessionCache.Cache.MyPlayer.User.posizione.ToVector3(), ConfigShared.SharedConfig.Main.Veicoli.gasstations[i].pos);
				if (dist < 100) cl = i;
				if (cl > 0)
		...
		public static async Task FuelCount()
		{
			try
			{
				...
				if (SessionCache.Cache.MyPlayer.User.StatiPlayer.InVeicolo && veh.Driver == playerPed && modelValid(veh) && !veh.IsDead)
				{
					if (LastVehicle != veh)
					{
						LastVehicle = veh;
				...
				else if (SessionCache.Cache.MyPlayer.User.StatiPlayer.InVeicolo && veh.Driver != playerPed && modelValid(veh) && !veh.IsDead)
				{
				...
				if (veh.Exists() || lastveh.Exists())
					for (int i = 0; i < ConfigShared.SharedConfig.Main.Veicoli.gasstations.Count; i++)
					{
						...
						if (dist <= 80f)
						{
							...
							if (fuelChecked == false && SessionCache.Cache.MyPlayer.User.StatiPlayer.InVeicolo)
							{
								fuelChecked = true;
							...
							for (int j = 0; j < ConfigShared.SharedConfig.Main.Veicoli.gasstations[i].pumps.Count; j++)
							{
								...
								float pdist = Vector3.Distance(SessionCache.Cache.MyPlayer.User.posizione.ToVector3(), ConfigShared.SharedConfig.Main.Veicoli.gasstations[i].pumps[j]);

								if (pdist < 3.05 && LastVehicle.Exists() && withinDist(SessionCache.Cache.MyPlayer.User.posizione.ToVector3(), LastVehicle) && !SessionCache.Cache.MyPlayer.User.StatiPlayer.InVeicolo)
								{
						...
						if (lastStation > 0)
						{
							float dista = Vector3.Distance(SessionCache.Cache.MyPlayer.User.posizione.ToVector3(), ConfigShared.SharedConfig.Main.Veicoli.gasstations[lastStation - 1].pos);
				...
				{
					float dist = Vector3.Distance(SessionCache.Cache.MyPlayer.User.posizione.ToVector3(), LastVehicle.Position);
		...
		public static async Task FuelTruck()
		{
			if (jobTruck.Handle == 0)
				...
				{
					float dist = Vector3.Distance(SessionCache.Cache.MyPlayer.User.posizione.ToVector3(), registrySpots[i]);
			...
			if (curRegPickup > 0)
			{
				...
				float dist = Vector3.Distance(SessionCache.Cache.MyPlayer.User.posizione.ToVector3(), spot);
			...
			{
				Vehicle vehicle = SessionCache.Cache.MyPlayer.Ped.CurrentVehicle;
				...
				if (vehicle == jobTruck && IsVehicleAttachedToTrailer(jobTruck.Handle))
				{
					...
					for (int i = 0; i < ConfigShared.SharedConfig.Main.Veicoli.gasstations.Count; i++)
					{
						float dis = Vector3.Distance(SessionCache.Cache.MyPlayer.User.posizione.ToVector3(), ConfigShared.SharedConfig.Main.Veicoli.gasstations[i].pos);
					...
					{
						float di = Vector3.Distance(SessionCache.Cache.MyPlayer.User.posizione.ToVector3(), refuelspots[i]);
				...
				float dist = Vector3.Distance(SessionCache.Cache.MyPlayer.User.posizione.ToVector3(), jobTruck.Position);
