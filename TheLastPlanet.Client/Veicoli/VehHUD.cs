...
namespace TheLastPlanet.Client.Veicoli
{
	internal static class VehHud
	{
		...
		private static async Task OnTickSpeedo3()
		{
			...
			else if (!Cache.MyPlayer.User.StatiPlayer.InVeicolo)
			{
				if (curAlpha <= 0)
					curAlpha = 0;
				else
		...
		{
			Client.Instance.NuiManager.SendMessage("buckle:seatbelts", new { transactionValue = value, inCar = Cache.MyPlayer.User.StatiPlayer.InVeicolo });
		}
