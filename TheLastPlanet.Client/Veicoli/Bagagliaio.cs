...
namespace TheLastPlanet.Client.Veicoli
{
	internal static class Bagagliaio
	{
		...
		{
			trunkOpen = false;
			if (CachePlayer.Cache.MyPlayer.Ped.LastVehicle != null) CachePlayer.Cache.MyPlayer.Ped.LastVehicle.Doors[VehicleDoorIndex.Trunk].Close();
		}
		...
		public static async Task ControlloBagagliaio()
		{
			...
			if (!CachePlayer.Cache.MyPlayer.Character.StatiPlayer.InVeicolo)
			{
				...
				if (distance < trunkdist)
				{
					...
					if (!trunkOpen && !CachePlayer.Cache.MyPlayer.Character.StatiPlayer.InVeicolo && !HUD.MenuPool.IsAnyMenuOpen)
					{
