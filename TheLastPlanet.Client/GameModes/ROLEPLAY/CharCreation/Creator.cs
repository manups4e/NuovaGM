...
using TheLastPlanet.Client.GameMode.ROLEPLAY.LogIn;
...
namespace TheLastPlanet.Client.GameMode.ROLEPLAY.CharCreation
{
    internal static class Creator
    {
        ...
        //TODO: MAYBE THIS HAS TO BE MOVED TO THE TELEPHONE SECTION?
        private static string[] Prefix = new string[6] { "333", "347", "338", "345", "329", "361" };
        private static Char_data _dataMale;
        private static Char_data _dataFemale;
        ...
        public static async Task CharCreationMenu(NewChar nuiData)
        {
            try
            {
                ...
                string name = nuiData.Name;
                string lastname = nuiData.LastName;
                string dob = nuiData.Dob;
                string sex = nuiData.Sex;
                Client.Instance.AddTick(Controls);
                _selected = sex;
                long insurance = SharedMath.GetRandomLong(999999999999999);
                ...
                ped_cre_board(_data);
                TaskWalkInToRoom(Cache.PlayerCache.MyPlayer.Ped, _selected == "Male" ? sub_7dd83(1, 0, "Male") : sub_7dd83(1, 0, "Female"));
                ...
                cam.Delete();
                if (!Creation.Visible) MenuCreazione(name, lastname, dob, sex);
        ...
        private static List<dynamic> _eyebArchList = new List<dynamic> { "Standard", "Alte", "Basse" };
        private static List<dynamic> _eyesList = new List<dynamic> { "Standard", "Grandi", "Stretti" };
        private static List<dynamic> _noseList = new List<dynamic> { "Standard", "Grande", "Piccolo" };
        ...
        public static void MenuCreazione(string nam, string lastnam, string dob, string sesso)
        {
            try
            {
                ...
                Creation = new UIMenu("TLP Creator", "Create a new Character", offset, "thelastgalaxy", "bannerbackground", false, true)
                {
                    ControlDisablingEnabled = true
                };
                UIMenuListItem sex = new UIMenuListItem("Sex", new List<dynamic>() { "Male", "Female" }, _selected == "Male" ? 0 : 1, "Select char sex");
                UIMenuItem InfoItem = new UIMenuItem("Character info", "First Name.. Last Name..");
                UIMenuItem ParentsItem = new UIMenuItem(GetLabelText("FACE_HERI"), GetLabelText("FACE_MM_H3"));
                UIMenuItem DetailsItem = new UIMenuItem(GetLabelText("FACE_FEAT"), GetLabelText("FACE_MM_H4"));
                UIMenuItem AppearancesItem = new UIMenuItem(GetLabelText("FACE_APP"), GetLabelText("FACE_MM_H6"));
                UIMenuItem ApparelItem = new UIMenuItem(GetLabelText("FACE_APPA"), GetLabelText("FACE_APPA_H"));

                Creation.AddItem(sex);
                Creation.AddItem(InfoItem);
                Creation.AddItem(ParentsItem);
                Creation.AddItem(DetailsItem);
                Creation.AddItem(AppearancesItem);
                Creation.AddItem(ApparelItem);

                Info = new("Character info", "Create a new Character")
                {
                    ControlDisablingEnabled = true
                };
                ...
                ParentsItem.BindItemToMenu(Parents);
                DetailsItem.BindItemToMenu(Details);
                AppearancesItem.BindItemToMenu(Appearances);
                ...
                name.SetRightLabel(nam);
                UIMenuItem lastname = new UIMenuItem("Last name");
                lastname.SetRightLabel(lastnam);
                UIMenuItem DoB = new UIMenuItem("Date of Birth");
                DoB.SetRightLabel(dob);
                UIMenuItem height = new UIMenuItem("Altezza");
                height.SetRightLabel("" + _data.Info.Height);
                Info.AddItem(name);
                ...
                Info.OnItemSelect += async (_menu, _item, _index) =>
                {
                    if (_item == name)
                    {
                        ...
                        string result = await HUD.GetUserInput("Insert char name", _data.Info.Firstname, 30);

                        if (result != null)
                        {
                            if (result.Length < 4)
                            {
                                HUD.ShowNotification("You inserted too short value! Please try again.", TheLastPlanet.Client.Core.Utility.HUD.ColoreNotifica.Red, true);
                            }
                            else
                            {
                                _data.Info.Firstname = result;
                                name.SetRightLabel(result);
                            }
                        }
                    }
                    ...
                    {
                        N_0x3ed1438c1f5c6612(2);
                        string result = await HUD.GetUserInput("Insert char last name", _data.Info.Lastname, 30);
                        ...
                        if (result != null)
                        {
                            ...
                            else
                            {
                                ...
                                lastname.SetRightLabel(result);
                            }
                        }
                    }
                    else if (_item == DoB)
                    {
                        string result = await HUD.GetUserInput("Insert date of birth", _data.Info.DateOfBirth, 30);

                        if (result != null)
                        {
                            if (result == "gg/mm/yyyy")
                            {
                                HUD.ShowNotification("Invalid date format!\n~b~Format~w~ must be 'gg/mm/yyyy'.", TheLastPlanet.Client.Core.Utility.HUD.ColoreNotifica.Red, true);
                            }
                    ...
                    if (_selected == "Male")
                        ...
                        _dataFemale = _data;
                    _boardScalep1.CallFunction("SET_BOARD", Client.Settings.RolePlay.Main.ServerName, _data.Info.Firstname + " " + _data.Info.Lastname, "Personaggio N°", "Powered by Manups4e", 0, Cache.PlayerCache.MyPlayer.User.Characters.Count + 1, 0);
                };
                ...
                Parents.OnListChange += async (_sender, _listItem, _newIndex) =>
                {
                    if (_listItem == mom)
                    {
                ...
                Apparel.OnIndexChange += async (sender, index) =>
                {
                    if (_data.Skin.Sex == "Male")
                    {
                        Dressing dress = new Dressing(SuitsMale[index].Name, SuitsMale[index].Description, SuitsMale[index].ComponentDrawables, SuitsMale[index].ComponentTextures, SuitsMale[index].PropIndices, SuitsMale[index].PropTextures);
                        _data.Dressing = dress;
                        _dataMale = _data;
                    }
                    else
                    {
                        Dressing dress = new Dressing(SuitsFemale[index].Name, SuitsFemale[index].Description, SuitsFemale[index].ComponentDrawables, SuitsFemale[index].ComponentTextures, SuitsFemale[index].PropIndices, SuitsFemale[index].PropTextures);
                        _data.Dressing = dress;
                        _dataFemale = _data;
                    }
                ...
                {
                    TaskCreaClothes(Cache.PlayerCache.MyPlayer.Ped, sub_7dd83(1, 0, _selected));

                    if (_selected == "Male")
                    {
                        for (int i = 0; i < SuitsMale.Count; i++)
                        {
                            UIMenuItem abito = new UIMenuItem(SuitsMale[i].Name, SuitsMale[i].Description);
                            Apparel.AddItem(abito);
                        }

                        UpdateDress(PlayerPedId(), SuitsMale[0]);
                    }
                    else
                    {
                        for (int i = 0; i < SuitsFemale.Count; i++)
                        {
                            UIMenuItem abito = new UIMenuItem(SuitsFemale[i].Name, SuitsFemale[i].Description);
                            Apparel.AddItem(abito);
                        }

                        UpdateDress(PlayerPedId(), SuitsFemale[0]);
                    }
                };
                ...
                UIMenuItem Save = new UIMenuItem("Confirm Character", "Ready to ~y~join the game~w~?", SColor.HUD_Freemode_dark, SColor.HUD_Freemode);
        ...
        private static async Task TaskHoldBoard()
        {
            int sequence = 0;
            OpenSequenceTask(ref sequence);
            TaskPlayAnim(PlayerPedId(), sub_7dd83(1, 0, _selected), "react_light", 8.0f, -8.0f, -1, 512, 0, false, false, false);
            TaskPlayAnim(PlayerPedId(), sub_7dd83(1, 0, _selected), "Loop", 8.0f, -8.0f, -1, 513, 0, false, false, false);
            CloseSequenceTask(sequence);
