...
namespace TheLastPlanet.Client.GameMode.ROLEPLAY.Vehicles
{
    internal static class VehiclesClient
    {
        ...
        public static Vehicle rentVehicle = new Vehicle(0);
        ...
        public static void Init()
        {
            ...
            {
                inputs.Add(new InputController(Control.Context, carGarageSpots[i].ToPosition(), "Press ~INPUT_CONTEXT~ to rent a vehicle", new((MarkerType)(-1), carGarageSpots[i].ToPosition(), SColor.Transparent), ServerMode.Roleplay, PadCheck.Any, ControlModifier.None, new Action<Ped, object[]>((playerPed, a) =>
                {
                    MenuRentVehicles.RentMenu((int)a[0]);
                }), i));
            }
        ...
        public static async Task Lux()
        {
            ...
            if (Cache.PlayerCache.MyPlayer.Status.PlayerStates.InVehicle && Main.spawned)
            {
                ...
                if (playerPed.SeatIndex == VehicleSeat.Driver)
                {
                    ...
                    if (Input.IsControlJustPressed(Control.DropAmmo, PadCheck.Keyboard))
                    {
                        if (veh.Speed < 10f && !veh.Model.IsBicycle)
                            ...
                            HUD.ShowNotification(veh.Model.IsBicycle ? "Bicycles have no engine!" : "You can't turn off the engine at this speed!");
                    }
        ...
        public static async Task vehHandle()
        {
            Ped playerPed = Cache.PlayerCache.MyPlayer.Ped;
            ...
            if (Main.spawned)
            {
                ...
                if (!IsPedInAnyVehicle(playerPed.Handle, false) && playerPed.LastVehicle != null && playerPed.LastVehicle.Exists() && !playerPed.LastVehicle.IsEngineRunning && turnedOn)
            ...
            if (Cache.PlayerCache.MyPlayer.Status.PlayerStates.InVehicle)
            {
                ...
                if (playerPed.SeatIndex == VehicleSeat.Driver)
                {
                    if (p.IsEngineRunning)
                    {
                        ...
                        p.IsDriveable = false;
                        turnedOn = false;
                        HUD.ShowNotification("Engine off.", ColoreNotifica.Cyan);
                    }
                    else
                    {
                        p.IsEngineRunning = true;
                        p.IsDriveable = true;
                        HUD.ShowNotification("Engine on.", ColoreNotifica.Cyan);
                        turnedOn = true;
                    }
                }
            }
        ...
        public static async void spawnRentVehicle(string model, int num)
        {
            ...
            if (!Functions.IsSpawnPointClear(new Vector3(carGarageSpawn[num].X, carGarageSpawn[num].Y, carGarageSpawn[num].Z), 5f))
            {
                if (Functions.IsSpawnPointClear(new Vector3(carGarageSpawnAlt[num][0].X, carGarageSpawnAlt[num][0].Y, carGarageSpawnAlt[num][0].Z), 5f))
                    spawn = carGarageSpawnAlt[num][0];
                else if (Functions.IsSpawnPointClear(new Vector3(carGarageSpawnAlt[num][1].X, carGarageSpawnAlt[num][1].Y, carGarageSpawnAlt[num][1].Z), 5f))
                    spawn = carGarageSpawnAlt[num][1];
                else if (Functions.IsSpawnPointClear(new Vector3(carGarageSpawnAlt[num][2].X, carGarageSpawnAlt[num][2].Y, carGarageSpawnAlt[num][2].Z), 5f))
                    spawn = carGarageSpawnAlt[num][2];
                else if (Functions.IsSpawnPointClear(new Vector3(carGarageSpawnAlt[num][3].X, carGarageSpawnAlt[num][3].Y, carGarageSpawnAlt[num][3].Z), 5f)) spawn = carGarageSpawnAlt[num][3];
            }
            else
            {
            ...
            Vehicle veicolo = await Functions.SpawnVehicleNoPlayerInside(model, new Vector3(spawn.X, spawn.Y, spawn.Z), spawn.W);
        ...
        {
            Vehicle[] vehs = Functions.GetVehiclesInArea(coords, 3f);
            ...
            if (Functions.IsSpawnPointClear(coords, 1f))
            {
                previewedVehicle = await Functions.SpawnLocalVehicle(name, coords, heading);
        ...
        public static void SaveVehicle(Vehicle veh)
        {
            ...
            blip.Name = MenuRentVehicles.rentedVeh.name + " (Rented)";
        }
		...
		public static async Task MostraMenuAffitto()
		{
			...
			if (!MenuHandler.IsAnyMenuOpen)
			{
				for (int i = 0; i < carGarageSpots.Count; i++)
				{
					...
					{
						HUD.ShowHelp("Press ~INPUT_CONTEXT~ per affittare un veicolo");
		...
		*/
        ...
        public static async Task AffittoInCorso()
        {
            ...
            if (MenuRentVehicles.rentedVeh != null)
            {
                if (Cache.PlayerCache.MyPlayer.User.Bank >= MenuRentVehicles.rentedVeh.price)
                {
                    BaseScript.TriggerServerEvent("lprp:removebank", MenuRentVehicles.rentedVeh.price);
                    HUD.ShowNotification("You paid $" + MenuRentVehicles.rentedVeh.price + " to rent the vehicle", ColoreNotifica.GreenLight, true);
                    BaseScript.TriggerServerEvent("lprp:serverlog", DateTime.Now.ToString("dd/MM/yyyy, HH:mm:ss") + $"Mr. {Cache.PlayerCache.MyPlayer.User.FullName}, [{Cache.PlayerCache.MyPlayer.Player.Name}], paid ${MenuRentVehicles.rentedVeh.price} for the renewal of the lease of a {MenuRentVehicles.rentedVeh.name}");
                }
                else
                {
                    HUD.ShowNotification("You do NOT have enough ~b~Money~w~ to cover the hourly rental rate for the Vehicle!", ColoreNotifica.Red, true);
                    await BaseScript.Delay(100);
                    HUD.ShowNotification("The Vehicle will stop working and will be returned to the relevant garage!");
                    rentVehicle.IsInvincible = true;
                    rentVehicle.FuelLevel = 0f;
                    rentVehicle.IsEngineRunning = false;
                    rentVehicle.IsDriveable = false;
                    if (rentVehicle.Model.IsBicycle) rentVehicle.IsPositionFrozen = true;
                    await BaseScript.Delay(30000);
                    rentVehicle.Delete(); // magari sostituire con soccorso
                    BaseScript.TriggerServerEvent("lprp:serverlog", DateTime.Now.ToString("dd/MM/yyyy, HH:mm:ss") + $"Mr. {Cache.PlayerCache.MyPlayer.User.FullName}, [{Cache.PlayerCache.MyPlayer.Player.Name}], was unable to pay the rent of ${MenuRentVehicles.rentedVeh.price} for the renewal of a {MenuRentVehicles.rentedVeh.name}, and the vehicle has been deleted");
                    await BaseScript.Delay(100);
                    MenuRentVehicles.rentedVeh = null;
                    HUD.ShowNotification("The vehicle you rented has been returned to the relevant garage.");
                }
            }
        }
    }
