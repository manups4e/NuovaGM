...
namespace TheLastPlanet.Client.GameMode.ROLEPLAY.Sport
{
    static class Yoga
    {
        ...
        private static string[] sequence1 = new string[7]
        {
        ...
        private static string[] sequence2 = new string[9]
        {
            "start_to_a1", "a1_pose", "a1_to_a2",
            "a2_pose", "a2_to_a3", "a3_pose",
        ...
        private static string[] sequence3 = new string[15]
        {
        ...
        {
            ConfigShared.SharedConfig.Main.Generics.ItemList["yogamatress"].Use -= async (item, index) =>
            {
                Matress = await Functions.CreateProp(new Model(YogaMatress), GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0, 2f, 0), new Vector3(0), true);
        ...
        {
            if (Matress != null && Matress.Exists())
            {
                if (Cache.PlayerCache.MyPlayer.Ped.IsInRangeOf(Matress.Position, 1.2f))
                {
                    HUD.ShowHelp("~INPUT_CONTEXT~ to practice Yoga\n~INPUT_FRONTEND_CANCEL~ to collect the mattress");
        ...
        private static void func_351(int iParam1)
        {
            ...
            if (!IsPedInjured(PlayerPedId()))
            {
                if (!IsEntityPlayingAnim(PlayerPedId(), "missfam5_yoga", "start_pose", 3))
                {
                    ...
                    {
                        Vector3 coord = Cache.PlayerCache.MyPlayer.Position.ToVector3;
