...
namespace TheLastPlanet.Client.GameMode.ROLEPLAY.Interactions
{
    internal static class SittingAnimations
    {
        ...
        public static async Task Televisione()
        {
            ...
            if (Seduto /*&&controllo per casa o hotel*/)
            {
                if (!stato)
                {
                    ...
                    if (Input.IsControlJustPressed(Control.Context))
                    {
                        Client.Instance.AddTick(Televisions.TVControls);
                        stato = true;
                    }
                ...
                {
                    Client.Instance.RemoveTick(Televisions.TVControls);
                    stato = false;
                    SetPedConfigFlag(PlayerPedId(), 414, false);
                }
            }
        ...
        {
            SediaClosest = World.GetAllProps().Where(o => Sedie.Contains((ObjectHash)(uint)o.Model.Hash)).FirstOrDefault(o => Cache.PlayerCache.MyPlayer.Position.Distance(o.Position) < 1.375f);
        ...
        public static async Task ChairsSit()
        {
            ...
            if (SediaClosest != null)
            {
                    ...
                    if (!Seduto)
                    {
                        ...
                        if (Vector3.Distance(SediaClosest.Position, ped.Position) < 0.35f)
                            if (!IsPedActiveInScenario(ped.Handle))
                                if (!p.IsDead && !(p.Health < 1) && !Cache.PlayerCache.MyPlayer.Status.PlayerStates.InVehicle)
                                {
                                    HUD.ShowHelp("Press ~INPUT_CONTEXT~ to sit");
                                    {
                                        if (Input.IsControlJustPressed(Control.Context))
                ...
                {
                    if (SediaClosest.IsPositionFrozen) SediaClosest.IsPositionFrozen = false;
                }

                if (Seduto)
                    if (IsPedActiveInScenario(PlayerPedId()))
                    {
                        HUD.ShowHelp("Press ~INPUT_CONTEXT~ to get up");
