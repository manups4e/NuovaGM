...
namespace TheLastPlanet.Client.GameMode.ROLEPLAY.Interactions
{
    public static class Prostitutes
    {
        ...
        {
            Prostituta = World.GetAllPeds().Select(o => new Ped(o.Handle)).Where(o => IsPedUsingScenario(o.Handle, "WORLD_HUMAN_PROSTITUTE_LOW_CLASS") || IsPedUsingScenario(o.Handle, "WORLD_HUMAN_PROSTITUTE_HIGH_CLASS")).FirstOrDefault(o => Vector3.Distance(Cache.PlayerCache.MyPlayer.Position.ToVector3, o.Position) < ProstDistance);
        ...
        public static async Task LoopProstitutes()
        {
            ...
            if (Prostituta != null)
            {
                ...
                if (Cache.PlayerCache.MyPlayer.Status.PlayerStates.InVehicle && p.CurrentVehicle.GetPedOnSeat(VehicleSeat.Passenger) != Prostituta)
                {
                    HUD.ShowHelp(GetLabelText("PROS_ACCEPT"));

                    if (Input.IsControlJustPressed(Control.VehicleHorn))
                    {
                        if (Cache.PlayerCache.MyPlayer.User.Money > 5f)
                        {
                            if (p.CurrentVehicle.ClassType != VehicleClass.Boats && p.CurrentVehicle.ClassType != VehicleClass.Cycles && p.CurrentVehicle.ClassType != VehicleClass.Motorcycles && p.CurrentVehicle.ClassType != VehicleClass.Helicopters && p.CurrentVehicle.ClassType != VehicleClass.Military && p.CurrentVehicle.ClassType != VehicleClass.Motorcycles && p.CurrentVehicle.ClassType != VehicleClass.Planes && p.CurrentVehicle.ClassType != VehicleClass.Trains)
                            {
                                if (p.CurrentVehicle.Speed < 1)
                                {
                                    if (p.CurrentVehicle.IsSeatFree(VehicleSeat.Passenger))
                                    {
                                        ...
                                        SetPedInVehicleContext(Prostituta.Handle, Functions.HashUint("MINI_PROSTITUTE_LOW_PASSENGER"));
