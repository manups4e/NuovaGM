...
namespace TheLastPlanet.Client.GameMode.ROLEPLAY.Giostre
{
    internal static class MontagneRusse
    {
        ...
        private static void Spawned(PlayerClient client) { Client.Instance.AddTick(EliminaGialli); }
        ...
        private static async Task EliminaGialli()
        {
            try
            {
                MRClosest = World.GetAllProps().Select(o => new Prop(o.Handle)).Where(o => DaEliminare.Contains((ObjectHash)(uint)o.Model.Hash)).FirstOrDefault(o => Cache.PlayerCache.MyPlayer.Position.Distance(o.Position) < 300f);
                        ...
                        if (ValoreIndiceSconosciuto != 0)
                        {
                            ...
                            if (SonoSeduto)
                            {
                                ...
                                if (Input.IsControlJustPressed(Control.FrontendX))
                                {
                                    ...
                                    {
                                        Cache.PlayerCache.MyPlayer.Ped.Task.PlayAnimation(RollerAnim, "hands_up_enter_player_" + Posto, 8f, -1, AnimationFlags.StayInEndFrame);
                                        while (IsEntityPlayingAnim(PlayerPedId(), RollerAnim, "hands_up_enter_player_" + Posto, 3)) await BaseScript.Delay(0);
                                        Cache.PlayerCache.MyPlayer.Ped.Task.PlayAnimation(RollerAnim, "hands_up_idle_a_player_" + Posto, 8f, -1, AnimationFlags.Loop);
                                    }
                                    else if (IsEntityPlayingAnim(PlayerPedId(), RollerAnim, "hands_up_idle_a_player_" + Posto, 3))
                                    {
                                        Cache.PlayerCache.MyPlayer.Ped.Task.PlayAnimation(RollerAnim, "hands_up_exit_player_" + Posto, 8f, -1, AnimationFlags.StayInEndFrame);
                                        while (IsEntityPlayingAnim(PlayerPedId(), RollerAnim, "hands_up_exit_player_" + Posto, 3)) await BaseScript.Delay(0);
                                        Cache.PlayerCache.MyPlayer.Ped.Task.PlayAnimation(RollerAnim, "safety_bar_grip_move_a_player_" + Posto, 8f, -1, AnimationFlags.Loop);
                                    }
                                }
                    ...
                    case "FERMATA":
                        ...
                        else
                        {
                            if (Attivato)
                            {
                                ...
                                if (SonoSeduto) BaseScript.TriggerServerEvent("lprp:montagnerusse:playerScende", Cache.PlayerCache.MyPlayer.Ped.NetworkId);
        ...
        private static async void playerScende(int playernetid)
        {
            ...
            if (personaggio != null)
            {
                ...
                if (personaggio.NetworkId == Cache.PlayerCache.MyPlayer.Ped.NetworkId) SonoSeduto = false;
        ...
        private static async Task ControlloMontagne()
        {
            foreach (Vector3 v in ingressi)
                if (Cache.PlayerCache.MyPlayer.Ped.IsInRangeOf(v, 1.3f) && Montagna.State == "ATTESA" && tempo > 0)
                {
                    HUD.ShowHelp("Press ~INPUT_CONTEXT~ per salire sulle montagne russe");
                    ...
                    if (Input.IsControlJustPressed(Control.Context))
                    {
                        ...
                        BaseScript.TriggerServerEvent("lprp:montagnerusse:playerSale", Cache.PlayerCache.MyPlayer.Ped.NetworkId, ingressi.IndexOf(v) / 2, Montagna.Carrelli[ingressi.IndexOf(v) / 2].Entity.NetworkId);
                    }
                }
            ...
            if (Cache.PlayerCache.MyPlayer.Ped.IsInRangeOf(new Vector3(-1646.863f, -1125.135f, 17.338f), 30f))
            {
        ...
        private static float func_49()
        {
            ...
            fVar3 = Functions.Rad2deg((float)Math.Asin(Functions.Deg2rad(fVar1 / fVar2)));

            return 10f * Functions.Rad2deg((float)Math.Sin(Functions.Deg2rad(fVar3)));
        }
        ...
        private static async void func_134(int iParam0, int iParam1, float fParam2)
        {
            ...
            if (Cache.PlayerCache.MyPlayer.Ped.IsInRangeOf(Montagna.Carrelli[0].Entity.Position, 50f))
        ...
        private static void func_135(Vector3 Param0, ref float uParam1, ref float uParam2, ref float uParam3, ref float uParam4)
        {
            ...
            fVar1 = Param0.Z / 2f;
            fVar2 = Param0.X / 2f;
            fVar3 = Functions.Rad2deg((float)Math.Sin(Functions.Deg2rad(fVar0)));
            fVar4 = Functions.Rad2deg((float)Math.Sin(Functions.Deg2rad(fVar1)));
            fVar5 = Functions.Rad2deg((float)Math.Sin(Functions.Deg2rad(fVar2)));
            fVar6 = Functions.Rad2deg((float)Math.Cos(Functions.Deg2rad(fVar0)));
            fVar7 = Functions.Rad2deg((float)Math.Cos(Functions.Deg2rad(fVar1)));
            fVar8 = Functions.Rad2deg((float)Math.Cos(Functions.Deg2rad(fVar2)));
