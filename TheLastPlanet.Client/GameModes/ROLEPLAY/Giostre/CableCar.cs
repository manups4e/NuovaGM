...
{
    internal static class CableCar
    {
        ...
        private static void CheckIfPlayerShouldBeKickedOut(TrenoFunivia treno)
        {
            ...
            float dist = Vector3.Distance(pos, Cache.PlayerCache.MyPlayer.Ped.Position);
        ...
        private static void KickPlayerOutOfMyCablecar(TrenoFunivia treno)
        {
            ...
            Cache.PlayerCache.MyPlayer.Ped.Position = Vector3.Add(Vector3.Add(treno.Position, right), new Vector3(0.0f, 0.0f, -5.3f));
        }
        ...
        private static void GivePlayerOptionToJoinMyCablecar(TrenoFunivia treno, bool moving)
        {
            ...
            {
                Vector3 plypos = Cache.PlayerCache.MyPlayer.Ped.Position;
                float dist = Vector3.Distance(pos, Cache.PlayerCache.MyPlayer.Ped.Position);

                if (dist < 3.0f)
                {
                    HUD.ShowHelp("Press ~INPUT_CONTEXT~ to enter the cab");

                    if (Input.IsControlJustPressed(Control.Context))
                    {
                        treno.Is_player_seated = true;
                        Cache.PlayerCache.MyPlayer.Ped.AttachTo(treno.Entity, Vector3.Subtract(Cache.PlayerCache.MyPlayer.Ped.Position, treno.Position), Cache.PlayerCache.MyPlayer.Ped.Rotation);
                    }
                }
            }
            else
            {
                if (!moving)
                {
                    HUD.ShowHelp("Press ~INPUT_CONTEXT~ to exit the cab");
