...
namespace TheLastPlanet.Client.GameMode.ROLEPLAY.Giostre
{
    internal static class RuotaPanoramica
    {
        private static RuotaPan Ruota = new RuotaPan(null, 0, "IDLE");
        ...
        private static Camera Cam1 = new Camera(0);
        private static RuotaPanoramicaCamTastiera Cam2Tastiera = new RuotaPanoramicaCamTastiera();
        private static RuotaPanoramicaCamGamePad Cam2GamePad = new RuotaPanoramicaCamGamePad();
        private static int iLocal_355 = 0;
        private static CabinaPan CabinaAttuale;
        private static CabinaPan[] Cabine = new CabinaPan[16] { new CabinaPan(0), new CabinaPan(1), new CabinaPan(2), new CabinaPan(3), new CabinaPan(4), new CabinaPan(5), new CabinaPan(6), new CabinaPan(7), new CabinaPan(8), new CabinaPan(9), new CabinaPan(10), new CabinaPan(11), new CabinaPan(12), new CabinaPan(13), new CabinaPan(14), new CabinaPan(15) };
        ...
        private static async void AggiornaGradient(int gradient) { Ruota.Gradient = gradient; }
        ...
        private static async Task SpawnaRuota()
        {
            ...
            while (i < 16)
            {
                ...
                Cabine[i].Entity = new Prop(CreateObject(GetHashKey("prop_ferris_car_01"), 0f, 1f, 2f, false, false, false)) { IsInvincible = true, Position = func_147(i), LodDistance = 1000, IsPositionFrozen = true };
                Cabine[i].Index = i;
                i++;
            }
        ...
        private static async Task MuoviRuota()
        {
            ...
            {
                float fVar2 = 0;
                if (iLocal_355 != 0) fVar2 = GetTimeDifference(GetNetworkTimeAccurate(), iLocal_355) / 800f;
                ...
                if (IsAudioSceneActive("FAIRGROUND_RIDES_FERRIS_WHALE")) SetAudioSceneVariable("FAIRGROUND_RIDES_FERRIS_WHALE", "HEIGHT", Cache.PlayerCache.MyPlayer.Ped.Position.Z - 13f);
                    ...
                    if (Math.Abs(Ruota.Rotazione - Cabine[i].Gradient) < 0.05f)
                    {
                        ...
                        switch (Ruota.State)
                        {
                            case "FACCIO_SALIRE":
                                ...
                                BaseScript.TriggerServerEvent("lprp:ruotapanoramica:playerSale", Cache.PlayerCache.MyPlayer.Ped.NetworkId, Ruota.Gradient);

                                break;
                            case "FACCIO_SCENDERE":
                                BaseScript.TriggerServerEvent("lprp:ruotapanoramica:playerScende", Cache.PlayerCache.MyPlayer.Ped.NetworkId, Ruota.Gradient);
        ...
        private static async void playerSale(int player, int cabina)
        {
            ...
            {
                if (Personaggio.NetworkId != Cache.PlayerCache.MyPlayer.Ped.NetworkId)
                ...
                Vector3 attCoords = GetOffsetFromEntityGivenWorldCoords(Cabina.Entity.Handle, Cache.PlayerCache.MyPlayer.Ped.Position.X, Cache.PlayerCache.MyPlayer.Ped.Position.Y, Cache.PlayerCache.MyPlayer.Ped.Position.Z);
                AttachEntityToEntity(Personaggio.Handle, Cabina.Entity.Handle, 0, attCoords.X, attCoords.Y, attCoords.Z, 0f, 0f, Cache.PlayerCache.MyPlayer.Ped.Heading, false, false, false, false, 2, true);
                ...
                if (Personaggio.Handle == PlayerPedId()) CreaCam();
        ...
        private static async void playerScende(int player, int cabina)
        {
            ...
            if (Personaggio == Cache.PlayerCache.MyPlayer.Ped)
            {
                ...
                Cabina.NPlayer = 0;
                BaseScript.TriggerServerEvent("lprp:ruotapanoramica:aggiornaCabine", Cabina.Index, Cabina.NPlayer);
                if (IsAudioSceneActive("FAIRGROUND_RIDES_FERRIS_WHALE")) StopAudioScene("FAIRGROUND_RIDES_FERRIS_WHALE");
                if (IsAudioSceneActive("FAIRGROUND_RIDES_FERRIS_WHALE_ALTERNATIVE_VIEW")) StopAudioScene("FAIRGROUND_RIDES_FERRIS_WHALE_ALTERNATIVE_VIEW");
                if (Personaggio.Handle == PlayerPedId()) GiroFinito = true;
                BaseScript.TriggerServerEvent("lprp:ruotapanoramica:RuotaFerma", false);
                Ruota.State = "IDLE";
                CabinaAttuale = null;
            }
        ...
        {
            if (Cache.PlayerCache.MyPlayer.Ped.IsInRangeOf(new Vector3(-1661.95f, -1127.011f, 12.6973f), 20f))
            {
                if (!IsAudioSceneActive("FAIRGROUND_RIDES_FERRIS_WHALE")) StartAudioScene("FAIRGROUND_RIDES_FERRIS_WHALE");

                if (Cache.PlayerCache.MyPlayer.Ped.IsInRangeOf(new Vector3(-1661.95f, -1127.011f, 12.6973f), 1.375f))
                {
                    HUD.ShowHelp("Press ~INPUT_CONTEXT~ per salire sulla prima gondola libera");
            ...
            if (!GiroFinito)
            {
                ...
                if (Input.IsControlJustPressed(Control.ScriptSelect)) CambiaCam();
            }
        ...
        private static bool Cambia = true;

        private static void CambiaCam()
        {
            Cambia = !Cambia;
        ...
        private static async void func_105(RuotaPanoramicaCamGamePad uParam0)
        {
            float uVar0 = 0;
            float uVar1 = 0;
            float fVar2;
            ...
            if (uParam0.Valore0)
            {
                if (Absf(GetControlNormal(2, 220)) > 0.1f)
                {
                    ...
                    if (uParam0.Valore15)
                    {
                        if (uParam0.Valore12 < -110) uParam0.Valore12 = -110;
                        if (uParam0.Valore12 > 110) uParam0.Valore12 = 110;
                    }
                    else
                    {
                        uParam0.Valore12 = func_102(uParam0.Valore12, -80f, 80f);
                    }
                }
                ...
                if (Absf(GetControlNormal(2, 221)) > 0.1f)
                {
                    ...
                    if (IsLookInverted()) fVar2 = fVar2 * -1f;
                    uParam0.Valore11 -= fVar2;

                    if (uParam0.Valore14)
                    {
                        if (uParam0.Valore11 < -30) uParam0.Valore11 = -30;
                        if (uParam0.Valore11 > 30) uParam0.Valore11 = 30;
                    }
                    else
                    {
                ...
                if (DoesCamExist(uParam0.CamEntity.Handle))
                {
                    ...
                    {
                        SetCamRot(uParam0.CamEntity.Handle, (Cache.PlayerCache.MyPlayer.Ped.Rotation + new Vector3(uParam0.Valore11, 0f, uParam0.Valore12)).X, (Cache.PlayerCache.MyPlayer.Ped.Rotation + new Vector3(uParam0.Valore11, 0f, uParam0.Valore12)).Y, (Cache.PlayerCache.MyPlayer.Ped.Rotation + new Vector3(uParam0.Valore11, 0f, uParam0.Valore12)).Z, 2);
                    }
        ...
        private static void func_106(Vector3 vParam0, Vector3 vParam1, ref float uParam2, ref float uParam3, int iParam4)
        {
            Vector3 vVar0;
        ...
        private static void func_107(Vector3 vParam0, ref float uParam1, ref float uParam2, int iParam3)
        {
            float fVar0;
            if (vParam0.Y != 0f)
        ...
        private static int func_109(RuotaPanoramicaCamGamePad uParam0, int iParam1, int iParam2)
        {
            if (!uParam0.Valore1) return 0;
            uParam0.Valore13 = 50f;
            if (!DoesCamExist(uParam0.CamEntity.Handle)) uParam0.CamEntity = new Camera(CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", uParam0.Valore2.X, uParam0.Valore2.Y, uParam0.Valore2.Z, uParam0.Valore5.X, uParam0.Valore5.Y, uParam0.Valore5.Z, 50f, true, 2));
            ...
            if (DoesCamExist(uParam0.CamEntity.Handle)) SetCamActive(uParam0.CamEntity.Handle, true);

            return 1;
        }

        private static void func_110(RuotaPanoramicaCamGamePad uParam0, bool bParam1)
        {
            ...
            uParam0.Valore11 = 0f;
            uParam0.Valore12 = 0f;
            if (bParam1) uParam0.Valore15 = true;
        }

        private static async void func_101(RuotaPanoramicaCamTastiera uParam0, bool bParam1, bool bParam2, bool bParam3, bool bParam4, float fParam5, bool bParam6, float fParam7, bool bParam8)
        {
            int[] iVar0 = new int[4];
            float fVar1;
            float fVar2;
            ...
            if (IsLookInverted()) iVar0[3] = iVar0[3] * -1;
            ...
            {
                if (uParam0.Valore28) uParam0.Valore17 = uParam0.Valore7;
            }
            else
            {
            ...
            if (bParam1)
            {
                ...
                else if (bParam8)
                {
                    ...
                    if (bParam3)
                    {
                        if (ToFloat(iVar0[3]) > 127f) uParam0.Valore17 -= (int)Math.Round(iVar0[3] / 128f * (uParam0.Valore19 / 2f));
                    }
                    else
                    {
                ...
                {
                    if (iVar0[1] < 0f) uParam0.Valore17 += (int)Math.Round(iVar0[1] / 128f * uParam0.Valore19);
                }
                else
                {
            ...
            uParam0.CamEntity.Rotation = Cache.PlayerCache.MyPlayer.Ped.Rotation + Cam2Tastiera.Valore14;
        }
        ...
        private static int func_103(int iParam0, int iParam1, int iParam2) { return iParam0 > iParam2 ? iParam2 : iParam0 < iParam1 ? iParam1 : iParam0; }

        private static void func_104(ref int uParam0, ref int uParam1, ref int uParam2, ref int uParam3, bool bParam4, bool bParam5)
        {
            ...
            if (bParam4)
            {
                if (!IsControlEnabled(2, 218)) uParam0 = Floor(GetDisabledControlUnboundNormal(2, 218) * 127f);
                if (!IsControlEnabled(2, 219)) uParam1 = Floor(GetDisabledControlUnboundNormal(2, 219) * 127f);
                if (!IsControlEnabled(2, 220)) uParam2 = Floor(GetDisabledControlUnboundNormal(2, 220) * 127f);
                if (!IsControlEnabled(2, 221)) uParam3 = Floor(GetDisabledControlUnboundNormal(2, 221) * 127f);
            }

            if (IsInputDisabled(2))
                if (bParam5)
                {
                    if (IsLookInverted()) uParam3 *= -1;
                    if (N_0xe1615ec03b3bb4fd()) uParam3 *= -1;
                }
        }

        private static void ForceState(string state) { Ruota.State = state; }

        private static Vector3 func_147(int iParam0)
        {
            float fVar0 = 6.28319f / 16 * iParam0;

            return GetOffsetFromEntityInWorldCoords(Ruota.Entity.Handle, 0f, Functions.Deg2rad(15.3f) * Functions.Rad2deg((float)Math.Sin(fVar0)), Functions.Deg2rad(-15.3f) * Functions.Rad2deg((float)Math.Cos(fVar0)));
        }
        ...
        private static void OnStop(string name)
        {
            if (name == GetCurrentResourceName())
                ...
                {
                    Ruota.Entity.Delete();
                    for (int i = 0; i < 16; i++) Cabine[i].Entity.Delete();
                }
        }
