...
{
    // never even started..
    internal static class BusinessShops
    {
        ...
        public static void NegozioPubblico(string tipo)
        {
            ...
            string description = "";
            List<ItemSell> oggettiDaAggiungere = Client.Settings.RolePlay.Shops.GenericShops.ItemsToSell.Shared;

            switch (tipo)
            {
                case "247":
                    description = "Aperti 24/7!";
                    Client.Settings.RolePlay.Shops.GenericShops.ItemsToSell.Tfs.ForEach(x => oggettiDaAggiungere.Add(x));

                    break;
                case "ltd":
                    description = "Non è mica infinita!";
                    Client.Settings.RolePlay.Shops.GenericShops.ItemsToSell.Ltd.ForEach(x => oggettiDaAggiungere.Add(x));

                    break;
                case "rq":
                    description = "I liquori migliori!";
                    Client.Settings.RolePlay.Shops.GenericShops.ItemsToSell.Rq.ForEach(x => oggettiDaAggiungere.Add(x));
			...
			for (int i = 0; i < stations.Count; i++)
			{
				...
				if (dist < 80)
				{
					...
					if (dist < 1.3f)
					{
						...
						{
							Funzioni.ShowHelp("Press ~INPUT_CONTEXT~ per gestire la stazione");
							if (!interactWait)
							{
