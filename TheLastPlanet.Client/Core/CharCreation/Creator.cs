...
namespace TheLastPlanet.Client.Core.CharCreation
{
	internal static class Creator
	{
			...
			try
			{
				...
				Info.OnItemSelect += async (_menu, _item, _index) =>
				{
					...
					if (_selezionato == "Maschio")
						...
						_dataFemmina = _data;
					_boardScalep1.CallFunction("SET_BOARD", Client.Impostazioni.Main.NomeServer, _data.info.firstname + " " + _data.info.lastname, "Personaggio N°", "Powered by Manups4e", 0, CachePlayer.Cache.MyPlayer.Character.Characters.Count + 1, 0);
				};
				...
				Salva.Activated += async (_selectedItem, _index) =>
				{
					...
					CachePlayer.Cache.MyPlayer.Character.char_current = _data.id;
		...
		private static async void CreaScaleform_Cre(Char_data data, Model overlay)
		{
			...
			_boardScalep1.CallFunction("SET_BOARD", "Nuova GM", data.info.firstname + " " + data.info.lastname, "Personaggio N°", "Powered by Manups4e", 0, CachePlayer.Cache.MyPlayer.Character.Characters.Count + 1, 0);
