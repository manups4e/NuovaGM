...
namespace TheLastPlanet.Client.TimeWeather
{
	internal static class Meteo
	{
		...
		{
			ClientSession.Instance.AddEventHandler("lprp:getMeteo", new Action<int, bool, bool>(SetMeteo));
			ClientSession.Instance.AddEventHandler("CambiaMeteoDinamicoPerTutti", new Action<bool>(SetDynamic));
		}
		...
		public static async Task PulisciVeicolo()
		{
			...
			if (!Transitioning)
				if (CurrentWeather == 7 || CurrentWeather == 8)
					Cache.MyPlayer.Ped.CurrentVehicle.DirtLevel -= 0.1f;
		}
	}
}