...
namespace TheLastPlanet.Client.Lavori.Whitelistati.Medici
{
	internal static class MenuMedici
	{
		...
		public static async void MenuSpogliatoio()
		{
			...
			UIMenuItem cambio;
			cambio = !CachePlayer.Cache.MyPlayer.Character.StatiPlayer.InServizio ? new UIMenuItem("Entra in Servizio", "Hai fatto un giuramento.") : new UIMenuItem("Esci dal Servizio", "Smetti di lavorare");
			...
			cambio.Activated += async (item, index) =>
			{
				...
				{
					foreach (KeyValuePair<string, JobGrade> Grado in Client.Impostazioni.Lavori.Medici.Gradi.Where(Grado => Grado.Value.Id == CachePlayer.Cache.MyPlayer.Character.CurrentChar.job.grade))
		...
		{
			CachePlayer.Cache.MyPlayer.Character.StatiPlayer.Istanza.Istanzia("SceltaVeicoliMedici");
		...
		public static async void HeliMenu(Ospedale Stazione, SpawnerSpawn Punto)
		{
			...
			MenuElicotteri.OnItemSelect += async (menu, item, index) =>
			{
				...
				CachePlayer.Cache.MyPlayer.Ped.CurrentVehicle.Mods.LicensePlate = Funzioni.GetRandomInt(99) + "MED" + Funzioni.GetRandomInt(999);
				CachePlayer.Cache.MyPlayer.Ped.CurrentVehicle.SetDecor("VeicoloMedici", Funzioni.GetRandomInt(100));
				if (CachePlayer.Cache.MyPlayer.Ped.CurrentVehicle.Model.Hash == 353883353) SetVehicleLivery(CachePlayer.Cache.MyPlayer.Ped.CurrentVehicle.Handle, 1);
				VeicoloPol veh = new(CachePlayer.Cache.MyPlayer.Ped.CurrentVehicle.Mods.LicensePlate, CachePlayer.Cache.MyPlayer.Ped.CurrentVehicle.Model.Hash, CachePlayer.Cache.MyPlayer.Ped.CurrentVehicle.Handle);
				BaseScript.TriggerServerEvent("lprp:polizia:AggiungiVehMedici", veh.SerializeToJson());
				HUD.MenuPool.CloseAllMenus();
