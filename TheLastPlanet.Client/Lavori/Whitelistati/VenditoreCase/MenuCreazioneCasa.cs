			...
			gestioneInteriorCasa.OnMenuStateChanged += async (_old, _new, _state) =>
			{
				...
				if (_new == gestioneInteriorCasa)
				{
					if (_state == MenuState.ChangeForward)
					{
						...
						SetPlayerControl(CachePlayer.Cache.MyPlayer.Player.Handle, false, 256);
						...
						if (MainCamera == null) MainCamera = World.CreateCamera(CachePlayer.Cache.MyPlayer.Character.posizione.ToVector3() + new Vector3(0, 0, 100), new Vector3(0, 0, 0), 45f);
			...
			HUD.MenuPool.OnMenuStateChanged += async (oldmenu, newmenu, state) =>
			{
				switch (state)
				{
					case MenuState.Opened:
						oldInstance = CachePlayer.Cache.MyPlayer.Character.StatiPlayer.Istanza;
						CachePlayer.Cache.MyPlayer.Character.StatiPlayer.Istanza.Istanzia("Creatore Immobiliare");

						break;
					case MenuState.Closed:
					{
						if (CachePlayer.Cache.MyPlayer.Character.StatiPlayer.Istanza.Instance == "Creatore Immobiliare") CachePlayer.Cache.MyPlayer.Character.StatiPlayer.Istanza.RimuoviIstanza();
						CachePlayer.Cache.MyPlayer.Character.StatiPlayer.Istanza = oldInstance;
					...
					case MenuState.ChangeForward when newmenu == selezionePunto:
					{
						...
						MainCamera.Position = CachePlayer.Cache.MyPlayer.Character.posizione.ToVector3() + new Vector3(0, 0, 100);
						MainCamera.IsActive = true;
						RenderScriptCams(true, false, 1000, true, true);
					...
					case MenuState.ChangeBackward when oldmenu == selezionePunto:
					{
						...
						SetPlayerControl(CachePlayer.Cache.MyPlayer.Player.Handle, true, 256);
						Screen.Fading.FadeIn(500);
