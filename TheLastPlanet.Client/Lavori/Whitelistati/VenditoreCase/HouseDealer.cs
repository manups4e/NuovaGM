...
namespace TheLastPlanet.Client.Lavori.Whitelistati.VenditoreCase
{
	internal static class HouseDealer
	{
		...
		{
			if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.job.name.ToLower() == "venditorecase") MenuCreazioneCasa.MenuCreazioneCase();
		}
		...
		private static async Task Markers()
		{
			...
			if (!CachePlayer.Cache.MyPlayer.Character.StatiPlayer.Istanza.Stanziato)
			{
				...
				{
					HUD.ShowHelp(CachePlayer.Cache.MyPlayer.Character.CurrentChar.job.name.ToLower() == "venditorecase" ? "Premi ~INPUT_CONTEXT~ per entrare in ufficio" : "Premi ~INPUT_CONTEXT~ per entrare nell'ufficio dell'agenzia immobiliare");

					if (Input.IsControlJustPressed(Control.Context))
					{
						Funzioni.Teleport(house.Config.Dentro);
						CachePlayer.Cache.MyPlayer.Character.StatiPlayer.Istanza.Istanzia("VenditoreCase");
					}
				}
			}
			else
			{
				if (CachePlayer.Cache.MyPlayer.Character.StatiPlayer.Istanza.Instance == "VenditoreCase")
				{
					...
					{
						HUD.ShowHelp(CachePlayer.Cache.MyPlayer.Character.CurrentChar.job.name.ToLower() == "venditorecase" ? "Premi ~INPUT_CONTEXT~ per uscire dall'ufficio" : "Premi ~INPUT_CONTEXT~ per uscire dall'ufficio dell'agenzia immobiliare");
			...
			if (CachePlayer.Cache.MyPlayer.Character.CurrentChar.job.name.ToLower() == "venditorecase")
		...
		private static async void MenuVenditoreCase()
		{
			...
			foreach (KeyValuePair<string, ConfigCase> app in Appartamenti.OrderBy(x => x.Value.Price))
			{
				...
				appartamento.OnMenuStateChanged += async (oldmenu, newmenu, state) =>
				{
					...
					newmenu.Clear();
					List<Player> players = Funzioni.GetPlayersInArea(CachePlayer.Cache.MyPlayer.Character.posizione.ToVector3(), 3.5f, false);
