...
namespace TheLastPlanet.Client.AdminAC
{
	internal static class DevManager
	{
		...
		public static async Task OnTickSviluppo()
		{
			...
			HUD.DrawText(0.4f, 0.925f, $"~o~Posizione~w~: {(Cache.PlayerCache.MyPlayer.User.StatiPlayer.RolePlayStates.InVeicolo ? pl.CurrentVehicle.Position : pl.Position)} H:{(Cache.PlayerCache.MyPlayer.User.StatiPlayer.RolePlayStates.InVeicolo ? pl.CurrentVehicle.Heading : pl.Heading)}");
			HUD.DrawText(0.4f, 0.95f, $"Rotazione: {(Cache.PlayerCache.MyPlayer.User.StatiPlayer.RolePlayStates.InVeicolo ? pl.CurrentVehicle.Rotation : pl.Rotation)}");
