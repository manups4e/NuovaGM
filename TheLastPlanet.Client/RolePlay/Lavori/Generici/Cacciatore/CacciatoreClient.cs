...
namespace TheLastPlanet.Client.RolePlay.Lavori.Generici.Cacciatore
{
	internal static class CacciatoreClient
	{
		...
		{
			if (Cache.MyPlayer.Ped.IsInRangeOf(Cacciatore.inizioCaccia, 1.375f))
			{
		...
		private static async void ControlloAnimale(int victim, int player, uint weaponHash, bool wasMeleeDamage)
		{
			...
			if (assassino.Character != Cache.MyPlayer.Ped) return;
		...
		private static async void ApriMenuAffittoArmi()
		{
			...
			if (!StaCacciando)
			{
				...
				{
					if (Cache.MyPlayer.User.HasLicense("Caccia"))
					{
						if (item == armi)
						{
							if ((Cache.MyPlayer.User.HasWeapon(DaFuoco) || affittatoFuoco) && (Cache.MyPlayer.User.HasWeapon(Bianca) || affittatoBianca))
							{
							...
							if (!Cache.MyPlayer.User.HasWeapon(DaFuoco))
								if (Cache.MyPlayer.User.Money >= 250 || Cache.MyPlayer.User.Bank >= 250)
								{
									Cache.MyPlayer.Ped.Weapons.Give(WeaponHash.SniperRifle, 100, false, true);
									prezzo += 250;
									affittatoFuoco = true;
								}

							if (!Cache.MyPlayer.User.HasWeapon(Bianca))
								if (Cache.MyPlayer.User.Money >= 50 || Cache.MyPlayer.User.Bank >= 50)
								{
									Cache.MyPlayer.Ped.Weapons.Give(WeaponHash.Knife, 1, false, true);
									prezzo += 50;
									affittatoBianca = true;
								}

							if (Cache.MyPlayer.User.Money >= prezzo)
							{
							...
							{
								if (Cache.MyPlayer.User.Bank >= prezzo)
								{
