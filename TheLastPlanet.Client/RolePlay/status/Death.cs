	...
	internal static class Death
	{
		...
		*/
		...
		public static async void StartDeathTimer()
		{
			...
			if (EarlyRespawn)
				if (EarlyRespawnFine)
					if (Cache.MyPlayer.User.Money >= EarlyRespawnFineAmount || Cache.MyPlayer.User.Bank >= EarlyRespawnFineAmount)
						canPayFine = true;
		...
		public static async void RemoveItemsAfterRPDeath()
		{
			...
			Vector3 pedCoords = Cache.MyPlayer.Ped.Position;
			Vector3 pos = hospitals.OrderBy(x => Vector3.Distance(pedCoords, x)).FirstOrDefault();
			Main.RespawnPed(new Position(pos.X, pos.Y, pos.Z, Cache.MyPlayer.Ped.Heading));
