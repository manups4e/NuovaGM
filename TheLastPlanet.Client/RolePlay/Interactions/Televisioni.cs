...
namespace TheLastPlanet.Client.RolePlay.Interactions
{
	...
	internal static class Televisioni
	{
		...
		{
			Prop tv = new Prop(GetClosestObjectOfType(Cache.MyPlayer.User.Posizione.ToVector3.X, Cache.MyPlayer.User.Posizione.ToVector3.Y, Cache.MyPlayer.User.Posizione.ToVector3.Z, 10f, (uint)World.GetAllProps().Select(o => new Prop(o.Handle)).Where(o => TVHashes.Contains((ObjectHash)(uint)o.Model.Hash)).First(o => Vector3.Distance(Cache.MyPlayer.User.Posizione.ToVector3, o.Position) < 5f).Model.Hash, false, false, true));
			...
			switch (iParam1)
			{
				...
				case 144897:
				case 145153:
					tv = new Prop(GetClosestObjectOfType(Cache.MyPlayer.User.Posizione.ToVector3.X, Cache.MyPlayer.User.Posizione.ToVector3.Y, Cache.MyPlayer.User.Posizione.ToVector3.Z, 5f, (uint)World.GetAllProps().Select(o => new Prop(o.Handle)).Where(o => TVHashes.Contains((ObjectHash)(uint)o.Model.Hash)).First(o => Vector3.Distance(Cache.MyPlayer.User.Posizione.ToVector3, o.Position) < 5f).Model.Hash, false, false, true));
