...
namespace TheLastPlanet.Client.RolePlay.Veicoli
{
	public static class FuelClient
	{
		...
		public static async void spawnTanker(Tanker spot, string plate)
		{
			...
			Cache.MyPlayer.Ped.SetIntoVehicle(jobTruck, VehicleSeat.Driver);
		...
		public static void FillFuel()
		{
			...
			{
				SetVehicleFuelLevel(Cache.MyPlayer.Ped.CurrentVehicle, Client.Impostazioni.RolePlay.Veicoli.DanniVeicoli.FuelCapacity);
		...
		public static async Task FuelCount()
		{
			try
			{
				...
				if (Cache.MyPlayer.User.StatiPlayer.InVeicolo && veh.Driver == playerPed && modelValid(veh) && !veh.IsDead)
				{
					if (LastVehicle != veh)
					{
						LastVehicle = veh;
				...
				if (veh.Exists() || lastveh.Exists())
					for (int i = 0; i < ConfigShared.SharedConfig.Main.Veicoli.gasstations.Count; i++)
					{
						...
						if (dist <= 80f)
						{
							...
							if (fuelChecked == false && Cache.MyPlayer.User.StatiPlayer.InVeicolo)
							{
								fuelChecked = true;
		...
		public static async Task FuelTruck()
		{
			...
			{
				Vehicle vehicle = Cache.MyPlayer.Ped.CurrentVehicle;
