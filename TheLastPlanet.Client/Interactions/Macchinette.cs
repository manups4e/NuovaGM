...
namespace TheLastPlanet.Client.Interactions
{
	internal static class Macchinette
	{
		...
		{
			VendingMachineClosest = World.GetAllProps().Select(o => new Prop(o.Handle)).Where(o => VendingHashes.Contains((ObjectHash)(uint)o.Model.Hash)).FirstOrDefault(o => Vector3.Distance(CachePlayer.Cache.MyPlayer.Character.posizione.ToVector3(), o.Position) < MachineRange);
		...
		public static async Task VendingMachines()
		{
			...
			if (VendingMachineClosest != null)
				...
				{
					if (CachePlayer.Cache.MyPlayer.Character.Money > 5)
					{
