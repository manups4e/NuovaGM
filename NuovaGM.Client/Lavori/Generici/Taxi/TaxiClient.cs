	...
	static class TaxiClient
	{
		...
		public static async Task Markers()
		{
			...
			if (p.IsInRangeOf(taxi.PosAccettazione, 100))
			{
				...
				{
					if(Eventi.Player.CurrentChar.job.name.ToLower() != "taxi")
					{
		...
		private static async Task ServizioTaxi()
		{
			...
			else if (jobs.onJob == 1 && InServizio)
			{
				if(VeicoloServizio.Exists() && VeicoloServizio.IsDriveable)
				{
							...
							else
							{
								if (jobs.flag[0] == 1 && jobs.flag[1] > 0)
								{
									...
									else
									{
										if (p.IsSittingInVehicle(VeicoloServizio))
										{
											if (Vector3.Distance(Eventi.Player.posizione.ToVector3(), NPCPasseggero.Position) < 8.0001f)
											{
						...
						else
						{
							...
							if (jobs.flag[0] == 0 && jobs.flag[1] > 0)
							{
								...
								jobs.flag[1]--;
								{
									if(jobs.flag[1] == 0)
									{
										Vector3 pos = Eventi.Player.posizione.ToVector3();
					...
					{
						if (Vector3.Distance(Eventi.Player.posizione.ToVector3(), VeicoloServizio.Position) > 30f)
							VaiFuoriServizio(1);
						else
