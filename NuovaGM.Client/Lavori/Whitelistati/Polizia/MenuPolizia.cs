	...
	public static class MenuPolizia
	{
		...
		public static async void CloakRoomMenu()
		{
			...
			*/
			...
			switch (Cache.Cache.MyPlayer.Character.StatiPlayer.InServizio)
			{
				...
				default:
				{
					if (Cache.Cache.MyPlayer.Character.StatiPlayer.InServizio || PoliziaMainClient.InServizioDaPilota)
					{
						Giubbotto = Cache.Cache.MyPlayer.Ped.Armor < 1 ? new UIMenuItem("Indossa il Giubbotto Anti-Proiettile", "Potrebbe salvarti la vita") : new UIMenuItem("Rimuovi il Giubbotto Anti-Proiettile", "Speriamo sia stato utile");
			...
			Spogliatoio.OnItemSelect += async (menu, item, index) =>
			{
				...
				if (item == Uniforme)
				{
					...
					{
						foreach (KeyValuePair<string, JobGrade> Grado in Client.Impostazioni.Lavori.Polizia.Gradi.Where(Grado => Cache.Cache.MyPlayer.Character.CurrentChar.job.name == "Polizia").Where(Grado => Grado.Value.Id == Cache.Cache.MyPlayer.Character.CurrentChar.job.grade))
				...
				{
					if (Cache.Cache.MyPlayer.Ped.Armor < 1)
					{
						SetPedComponentVariation(PlayerPedId(), 9, 4, 1, 2);
						Cache.Cache.MyPlayer.Ped.Armor = 30;
					}
					else
					{
						SetPedComponentVariation(PlayerPedId(), 9, 0, 1, 2);
						Cache.Cache.MyPlayer.Ped.Armor = 0;
					}
				}
			...
			HUD.MenuPool.OnMenuStateChanged += async (_oldMenu, _newMenu, state) =>
			{
				if (_newMenu == DatiPlayer)
				{
					...
					if (Client.Instance.GetPlayers.ToList().Count > 1)
					{
						...
						if (distance < 3f && ClosestPed != null)
						{
							...
							sesso.SetRightLabel(Cache.Cache.MyPlayer.Character.CurrentChar.skin.sex);
							UIMenuItem altezza = new("Altezza");
							altezza.SetRightLabel(Cache.Cache.MyPlayer.Character.CurrentChar.info.height + "cm");
							UIMenuItem job = new("Occupazione Attuale");
							job.SetRightLabel(Cache.Cache.MyPlayer.Character.CurrentChar.job.name);
							UIMenuItem telefono = new("N° di Telefono");
							telefono.SetRightLabel("" + Cache.Cache.MyPlayer.Character.CurrentChar.info.phoneNumber);
							UIMenuItem assicurazione = new("N° di Assicurazione");
							assicurazione.SetRightLabel("" + Cache.Cache.MyPlayer.Character.CurrentChar.info.insurance);
				...
				else if (_newMenu == Perquisizione)
				{
					...
					if (Client.Instance.GetPlayers.ToList().Count() > 1)
					{
						...
						if (distance < 3f)
						{
							List<Inventory> inv = Cache.Cache.MyPlayer.Character.Inventory;
						...
						switch (item)
						{
							...
							case UIMenuItem i when i == accompagna:
								if (player.StatiPlayer.Ammanettato) // rifare client-->server-->client
									BaseScript.TriggerServerEvent("lprp:polizia:accompagna", playerServerId, Cache.Cache.MyPlayer.Ped.NetworkId);
								else
			...
			InterazioneVeicolo.OnItemSelect += async (menu, item, index) =>
			{
				switch (item)
				{
					case UIMenuItem n when n == PickLock:
						...
						Cache.Cache.MyPlayer.Ped.Task.PlayAnimation("anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 8f, -1, AnimationFlags.Loop);
		...
		public static async void HeliMenu(StazioniDiPolizia Stazione, SpawnerSpawn Punto)
		{
			...
			MenuElicotteri.OnItemSelect += async (menu, item, index) =>
			{
				...
				Cache.Cache.MyPlayer.Ped.CurrentVehicle.Mods.LicensePlate = Funzioni.GetRandomInt(99) + "POL" + Funzioni.GetRandomInt(999);
				if (Cache.Cache.MyPlayer.Ped.CurrentVehicle.Model.Hash == 353883353) SetVehicleLivery(Cache.Cache.MyPlayer.Ped.CurrentVehicle.Handle, 0);
				Cache.Cache.MyPlayer.Ped.CurrentVehicle.SetDecor("VeicoloPolizia", Funzioni.GetRandomInt(100));
				VeicoloPol veh = new VeicoloPol(Cache.Cache.MyPlayer.Ped.CurrentVehicle.Mods.LicensePlate, Cache.Cache.MyPlayer.Ped.CurrentVehicle.Model.Hash, Cache.Cache.MyPlayer.Ped.CurrentVehicle.Handle);
		...
		{
			Cache.Cache.MyPlayer.Character.StatiPlayer.Istanza.Istanzia("SceltaVeicoliPolizia");
		...
		private static async Task ControlloMenu()
		{
			...
			if (Cache.Cache.MyPlayer.Character.StatiPlayer.InVeicolo)
			{
				if (p.CurrentVehicle.Driver == Cache.Cache.MyPlayer.Ped && p.CurrentVehicle.Speed < 2 || p.CurrentVehicle.GetPedOnSeat(VehicleSeat.Passenger) == Cache.Cache.MyPlayer.Ped)
				{
