...
namespace NuovaGM.Client.Giostre
{
	static class Funivia
	{
		...
		static float[] Speeds = new float[2] { 1.0f, .05f };

		static Dictionary<int, List<Vector3>> Tracks = new Dictionary<int, List<Vector3>>
		{
			[0] = new List<Vector3>
			{
		...
		static TrenoFunivia[] Cable_cars = new TrenoFunivia[2]
		{
			new TrenoFunivia(null, null, null, null, null, 0, new Vector3(0), 1, 0, 0, 0, 0, 0, 0, true, false, 0.65f, "IDLE", new Vector3(-0.2f, 0.0f, 0.0f)),
			new TrenoFunivia(null, null, null, null, null, 1, new Vector3(0), 1, 0, 0, 0, 0, 0, 0, true, false, 0.65f, "IDLE", new Vector3(-0.2f, 0.0f, 0.0f))
		};

		public static void Init()
		{
			Blip Sopra = new Blip(AddBlipForCoord(446.8f, 5571.1f, 780.7f))
			{
				Sprite = BlipSprite.Lift,
				IsShortRange = true,
				Name = "Funivia"
			};
			Blip Sotto = new Blip(AddBlipForCoord(-740.3f, 5594.5f, 41.2f))
			{
				Sprite = BlipSprite.Lift,
				IsShortRange = true,
				Name = "Funivia"
		...
		static void OnStop(string name)
		{
			...
			{
				if (Cable_cars[0].Is_player_seated)
				{
					KickPlayerOutOfMyCablecar(Cable_cars[0]);
				}

				if (Cable_cars[1].Is_player_seated)
				{
		...
		private static async Task UpdateCableMovement()
		{
			for (int i = 0; i < Cable_cars.Length; i++)
			{
				if (Cable_cars[i].State == "MOVE_UP")
				{
					...
					if (Cable_cars[i].Gradient_distance > 30.0)
					{
						zLerp = (-1.0f + Math.Abs(Lerp(1.0f, -1.0f, Cable_cars[i].Run_timer))) * 0.25f;
					}
				...
				else if (Cable_cars[i].State == "MOVE_DOWN")
				{
					...
					if (Cable_cars[i].Gradient_distance > 20.0)
					{
						zLerp = (-1.0f + Math.Abs(Lerp(1.0f, -1.0f, Cable_cars[i].Run_timer))) * 0.25f;
					}
		...
		static void ReleaseRunningSound(TrenoFunivia treno)
		{
			if (treno.Audio != -1)
			{
		...
		static void SetCablecarDoors(TrenoFunivia treno, bool state)
		{
		...
		static int WhatDirectionDoesMyCablecarGo(TrenoFunivia treno)
		{
			if (treno.Index == 0)
			{
		...
		static int UpdateCablecarGradient(TrenoFunivia treno)
		{
		...
		{
			return degs * 3.141592653589793f / 180;
		}
		...
		static Vector3 VecLerp(float x1, float y1, float z1, float x2, float y2, float z2, float l, bool clamp)
		{
			if (clamp)
			{
				if (l < 0.0)
