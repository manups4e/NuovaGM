...
namespace TheLastPlanet.Client.Interactions
{
	internal static class Docce
	{
		...
		public static async Task ControlloDocceVicino()
		{
			if (!InDoccia)
			{
				VicinoDoccia = World.GetAllProps().Select(o => new Prop(o.Handle)).Where(o => Doccie.Contains(o.Model.Hash)).Any(o => Vector3.Distance(Cache.Cache.MyPlayer.Character.posizione.ToVector3(), o.Position) < 1.375f);

				if (VicinoDoccia)
				{
					DocciaPorta = World.GetAllProps().Select(o => new Prop(o.Handle)).Where(o => Doccie.Contains(o.Model.Hash)).First(o => Vector3.Distance(Cache.Cache.MyPlayer.Character.posizione.ToVector3(), o.Position) < 1.375f);
			...
			if (VicinoDoccia)
				if (!InDoccia)
				{
					...
					if (Input.IsControlJustPressed(Control.Context))
					{
						...
						if (DoesEntityHaveDrawable(DocciaPorta.Handle)) PlaySynchronizedEntityAnim(DocciaPorta.Handle, Scena1, sLocal_445, sLocal_436, 1000f, -8f, 1, 1148846080);
						...
						if (Cache.Cache.MyPlayer.Character.CurrentChar.skin.sex == "Femmina")
						{
		...
		private static void func_314()
		{
			...
			if (GetPedDrawableVariation(PlayerPedId(), 3) != 15) SetPedComponentVariation(PlayerPedId(), 3, 15, 0, 0);
			...
			if (GetPedDrawableVariation(PlayerPedId(), 6) != 5) SetPedComponentVariation(PlayerPedId(), 6, 5, 0, 0);
			ClearAllPedProps(PlayerPedId());

			if (GetEntityModel(PlayerPedId()) == GetHashKey("mp_m_freemode_01"))
			{
				if (GetPedDrawableVariation(PlayerPedId(), 4) != 14) SetPedComponentVariation(PlayerPedId(), 4, 14, 0, 0);
			}
			...
			if (HasPedHeadBlendFinished(PlayerPedId()) && HasStreamedPedAssetsLoaded(PlayerPedId())) N_0x4668d80430d6c299(PlayerPedId());
		}
	}
