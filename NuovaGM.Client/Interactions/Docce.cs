...
namespace TheLastPlanet.Client.Interactions
{
	internal static class Docce
	{
		...
		public static async Task ControlloDocceVicino()
		{
			if (!InDoccia)
			{
				VicinoDoccia = World.GetAllProps().Select(o => new Prop(o.Handle)).Where(o => Doccie.Contains(o.Model.Hash)).Any(o => Vector3.Distance(Cache.Char.posizione.ToVector3(), o.Position) < 1.375f);

				if (VicinoDoccia)
				{
					DocciaPorta = World.GetAllProps().Select(o => new Prop(o.Handle)).Where(o => Doccie.Contains(o.Model.Hash)).First(o => Vector3.Distance(Cache.Char.posizione.ToVector3(), o.Position) < 1.375f);
			...
			if (VicinoDoccia)
				if (!InDoccia)
				{
					...
					if (Input.IsControlJustPressed(Control.Context))
					{
						...
						if (Cache.Char.CurrentChar.skin.sex == "Femmina")
						{
