...
//			await BaseScript.Delay(10000);
			if (MenuAffittoVeicoli.veicoloInAffitto != null)
			{
				if (Game.Player.GetPlayerData().Bank >= MenuAffittoVeicoli.veicoloInAffitto.price)
				{
					...
					BaseScript.TriggerServerEvent("lprp:serverlog", DateTime.Now.ToString("dd/MM/yyyy, HH:mm:ss") + $"Il Signor {Game.Player.GetPlayerData().FullName}, [{GetPlayerName(Convert.ToInt32(Game.Player.GetPlayerData().source))}], ha pagato ${MenuAffittoVeicoli.veicoloInAffitto.price} per il rinnovo dell'affitto di un/a {MenuAffittoVeicoli.veicoloInAffitto.name}");
				}
				else
				{
					...
					BaseScript.TriggerServerEvent("lprp:serverlog", DateTime.Now.ToString("dd/MM/yyyy, HH:mm:ss") + $"Il Signor {Game.Player.GetPlayerData().FullName}, [{GetPlayerName(Convert.ToInt32(Game.Player.GetPlayerData().source))}], non ha potuto pagare l'affitto di ${MenuAffittoVeicoli.veicoloInAffitto.price} per il rinnovo di un/a {MenuAffittoVeicoli.veicoloInAffitto.name}, ed il veicolo è stato eliminato");
					await BaseScript.Delay(100);
