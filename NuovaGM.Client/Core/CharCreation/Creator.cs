...
namespace TheLastPlanet.Client.Core.CharCreation
{
	internal static class Creator
	{
		...
		public static async void CharCreationMenu(IDictionary<string, object> nuiData)
		{
			try
			{
				_dummyPed = await Funzioni.CreatePedLocally(PedHash.FreemodeFemale01, Cache.PlayerPed.Position + new Vector3(10));
				...
				ped_cre_board(_data);
				TaskWalkInToRoom(Cache.PlayerPed, _selezionato == "Maschio" ? sub_7dd83(1, 0, "Maschio") : sub_7dd83(1, 0, "Femmina"));
			...
			try
			{
				...
				Info.OnItemSelect += async (_menu, _item, _index) =>
				{
					...
					if (_selezionato == "Maschio")
						...
						_dataFemmina = _data;
					_boardScalep1.CallFunction("SET_BOARD", Client.Impostazioni.Main.NomeServer, _data.info.firstname + " " + _data.info.lastname, "Personaggio N°", "Powered by Manups4e", 0, Cache.Char.char_data.Count + 1, 0);
				};
				...
				Apparel.OnIndexChange += async (sender, index) =>
				{
					...
					TaskProvaClothes(Cache.PlayerPed, sub_7dd83(1, 0, _data.skin.sex));
				};
				...
				HUD.MenuPool.OnMenuStateChanged += async (_oldMenu, _newMenu, state) =>
				{
					switch (state)
					{
						case MenuState.ChangeForward:
						{
							...
							{
								TaskCreaClothes(Cache.PlayerPed, sub_7dd83(1, 0, _selezionato));
						...
						case MenuState.ChangeBackward:
						{
							...
							{
								Apparel.Clear();
								TaskClothesALoop(Cache.PlayerPed, sub_7dd83(1, 0, _selezionato));
							}
				...
				Salva.Activated += async (_selectedItem, _index) =>
				{
					...
					BaseScript.TriggerServerEvent("lprp:finishCharServer", _data.Serialize());
					Cache.Char.char_current = _data.id;
		...
		private static async void CreaScaleform_Cre(Char_data data, Model overlay)
		{
			...
			_boardScalep1.CallFunction("SET_BOARD", "Nuova GM", data.info.firstname + " " + data.info.lastname, "Personaggio N°", "Powered by Manups4e", 0, Cache.Char.char_data.Count + 1, 0);
		...
		private static void sub_7cddb()
		{
			...
			if (!Cache.PlayerPed.IsInjured) SetFacialIdleAnimOverride(Cache.PlayerPed.Handle, v_3, "0");
		}
