...
namespace TheLastPlanet.Client.Core.CharCreation
{
	static class Creator
	{
			...
			try
			{
				...
				Info.OnItemSelect += async (_menu, _item, _index) =>
				{
					...
					if (selezionato == "Maschio")
						...
						dataFemmina = data;
					board_scalep1.CallFunction("SET_BOARD", Client.Impostazioni.Main.NomeServer, data.info.firstname + " " + data.info.lastname, "Personaggio N°", "Powered by Manups4e", 0, Cache.Char.char_data.Count + 1, 0);
				};
				#endregion
		...
		static async void CreaScaleform_Cre(Char_data data, Model overlay)
		{
			...
			board_scalep1.CallFunction("SET_BOARD", "Nuova GM", data.info.firstname + " " + data.info.lastname, "Personaggio N°", "Powered by Manups4e", 0, Cache.Char.char_data.Count + 1, 0);
