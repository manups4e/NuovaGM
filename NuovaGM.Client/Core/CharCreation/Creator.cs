...
namespace TheLastPlanet.Client.Core.CharCreation
{
	internal static class Creator
	{
		...
		public static async void CharCreationMenu(IDictionary<string, object> nuiData)
		{
			try
			{
				_dummyPed = await Funzioni.CreatePedLocally(PedHash.FreemodeFemale01, Cache.Cache.MyPlayer.Ped.Position + new Vector3(10));
				...
				Cache.Cache.MyPlayer.Ped.Position = new Vector3(402.91f, -996.74f, -180.00025f);
				while (!HasCollisionLoadedAroundEntity(Cache.Cache.MyPlayer.Ped.Handle)) await BaseScript.Delay(1);
				...
				ped_cre_board(_data);
				TaskWalkInToRoom(Cache.Cache.MyPlayer.Ped, _selezionato == "Maschio" ? sub_7dd83(1, 0, "Maschio") : sub_7dd83(1, 0, "Femmina"));
			...
			try
			{
				...
				Sesso.OnListChanged += async (item, _newIndex) =>
				{
					...
					switch (_newIndex)
					{
						case 0:
							_dataFemmina = _data;
							_data = _dataMaschio;
							_boardScalep1.CallFunction("SET_BOARD", Client.Impostazioni.Main.NomeServer, _data.info.firstname + " " + _data.info.lastname, "Personaggio N°", "Powered by Manups4e", 0, _data.id, 0);
				...
				Info.OnItemSelect += async (_menu, _item, _index) =>
				{
					...
					if (_selezionato == "Maschio")
						...
						_dataFemmina = _data;
					_boardScalep1.CallFunction("SET_BOARD", Client.Impostazioni.Main.NomeServer, _data.info.firstname + " " + _data.info.lastname, "Personaggio N°", "Powered by Manups4e", 0, Cache.Cache.MyPlayer.Character.Characters.Count + 1, 0);
				};
				...
				Apparel.OnIndexChange += async (sender, index) =>
				{
					...
					TaskProvaClothes(Cache.Cache.MyPlayer.Ped, sub_7dd83(1, 0, _data.skin.sex));
				};
				...
				HUD.MenuPool.OnMenuStateChanged += async (_oldMenu, _newMenu, state) =>
				{
					switch (state)
					{
						case MenuState.ChangeForward:
						{
							...
							{
								TaskCreaClothes(Cache.Cache.MyPlayer.Ped, sub_7dd83(1, 0, _selezionato));
						...
						case MenuState.ChangeBackward:
						{
							...
							{
								Apparel.Clear();
								TaskClothesALoop(Cache.Cache.MyPlayer.Ped, sub_7dd83(1, 0, _selezionato));
							}
				...
				Salva.Activated += async (_selectedItem, _index) =>
				{
					...
					Cache.Cache.MyPlayer.Character.char_current = _data.id;
		...
		public static async void Pol_Board2(Char_data data)
		{
			...
			BD1.AttachTo(Cache.Cache.MyPlayer.Ped.Bones[Bone.PH_R_Hand], Vector3.Zero, Vector3.Zero);
		...
		private static async void CreaScaleform_Cre(Char_data data, Model overlay)
		{
			...
			_boardScalep1.CallFunction("SET_BOARD", "Nuova GM", data.info.firstname + " " + data.info.lastname, "Personaggio N°", "Powered by Manups4e", 0, Cache.Cache.MyPlayer.Character.Characters.Count + 1, 0);
			_handle1 = CreateNamedRenderTargetForModel("ID_Text", (uint)overlay.Hash);
		}

		public static async Task Scaleform()
		{
			if (Cache.Cache.MyPlayer.Ped.Exists())
			{
		...
		private static void sub_7cddb()
		{
			...
			if (!Cache.Cache.MyPlayer.Ped.IsInjured) SetFacialIdleAnimOverride(Cache.Cache.MyPlayer.Ped.Handle, v_3, "0");
		}
