...
namespace NuovaGM.Client.gmPrincipale.MenuGm
{
	static class Menus
	{
		...
		public static async void CharSelectionMenu()
		{
			...
			CharSelection.Clear();
			if (Eventi.Player.char_data.Count > 0)
			{
				for (int i = 0; i < Eventi.Player.char_data.Count; i++)
				{
					Char_data pers = Eventi.Player.char_data[i];
					UIMenu Personaggio = HUD.MenuPool.AddSubMenu(CharSelection, Eventi.Player.char_data[i].info.firstname + " " + Eventi.Player.char_data[i].info.lastname);
					string morto = "";
					if (!pers.is_dead)
					{
					...
					seleziona.Activated += async (menu, item) =>
					{
						...
						await BaseScript.Delay(1000);
						Eventi.Player.char_current = pers.id;
			...
			if (Eventi.Player.char_data.Count >= 0 && Eventi.Player.char_data.Count < 3)
			{
		...
		public static async void CharCreationMenu()
		{
			...
			{
				Shared.Log.Printa(LogType.Error, "CharCreationMenu = " + ex);
			}
		}
		...
		public static async void MenuCreazione()
		{
			try
			{
				...
				Salva.Activated += async (_selectedItem, _index) =>
				{
					...
					Eventi.Player.char_current = data.id;
					BaseScript.TriggerServerEvent("lprp:updateCurChar", "char_current", Eventi.Player.char_current);
					Client.GetInstance.DeregisterTickHandler(Controllo);
					Client.GetInstance.DeregisterTickHandler(Scaleform);
					Client.GetInstance.DeregisterTickHandler(TastiMenu);
		...
		static async void CreaScaleform_Cre(Char_data data, Model overlay)
		{
			...
			board_scalep1.CallFunction("SET_BOARD", "Nuova GM", data.info.firstname + " " + data.info.lastname, "Personaggio N°", "Powered by Manups4e", 0, Eventi.Player.char_data.Count + 1, 0);
