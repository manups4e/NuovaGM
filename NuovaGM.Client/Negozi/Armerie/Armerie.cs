			...
			HUD.MenuPool.OnMenuStateChanged += async (oldmenu, newmenu, state) =>
			{
				...
				else if (state == MenuState.ChangeForward && newmenu == ArmiLic1)
				{
					...
					for (int i = 0; i < armi1.Count; i++)
					{
						...
						if (Cache.Cache.MyPlayer.Character.Money >= armi1[i].price || Cache.Cache.MyPlayer.Character.Bank >= armi1[i].price)
					...
					ArmiLic1.OnItemSelect += (_menu, _item, _index) =>
					{
						...
						{
							if (Cache.Cache.MyPlayer.Character.Money >= 150)
							{
							...
							{
								if (Cache.Cache.MyPlayer.Character.Bank >= 150)
								{
									AddAmmoToPed(playerPed.Handle, Funzioni.HashUint(armi1[_index].name), 250);
									HUD.ShowNotification("Poichè già possiedi quest'arma, ti sono state ricaricate le munizioni al prezzo di 150$");
						...
						{
							if (Cache.Cache.MyPlayer.Character.Money >= armi1[_index].price)
							{
				...
				else if (state == MenuState.ChangeForward && newmenu == ArmiLic2)
				{
					...
					for (int i = 0; i < armi2.Count; i++)
					{
						...
						if (Cache.Cache.MyPlayer.Character.Money >= armi2[i].price || Cache.Cache.MyPlayer.Character.Bank >= armi2[i].price)
					...
					ArmiLic2.OnItemSelect += (_menu, _item, _index) =>
					{
						...
						{
							if (Cache.Cache.MyPlayer.Character.Money >= armi2[_index].price)
							{
				...
				else if (state == MenuState.ChangeForward && newmenu == ArmiLic3)
				{
					...
					for (int i = 0; i < armi3.Count; i++)
					{
						...
						if (Cache.Cache.MyPlayer.Character.Money >= armi3[i].price || Cache.Cache.MyPlayer.Character.Bank >= armi3[i].price)
					...
					ArmiLic3.OnItemSelect += (_menu, _item, _index) =>
					{
						...
						{
							if (Cache.Cache.MyPlayer.Character.Money >= armi3[_index].price)
							{
				...
				else if (state == MenuState.ChangeForward && newmenu == component)
				{
					...
					foreach (Weapons armi in Cache.Cache.MyPlayer.Character.CurrentChar.weapons)
						if (SharedScript.hasComponents(armi.name))
						{
							...
							foreach (ArmiLicenza co in componenti)
								if (SharedScript.hasWeaponComponent(armi.name, co.name))
								{
									...
									Arma.AddItem(compon);
									if (Cache.Cache.MyPlayer.Character.Money >= co.price || Cache.Cache.MyPlayer.Character.Bank >= co.price)
							...
							Arma.OnItemSelect += async (_menu, _item, _index) =>
							{
								...
								else
								{
									...
									if (Cache.Cache.MyPlayer.Character.Money >= arm.price)
									{
									...
									{
										if (Cache.Cache.MyPlayer.Character.Bank >= arm.price)
										{
											BaseScript.TriggerServerEvent("lprp:addWeaponComponent", armi.name, arm.name);
				...
				else if (state == MenuState.ChangeForward && newmenu == Tinte)
				{
					...
					foreach (Weapons armi in Cache.Cache.MyPlayer.Character.CurrentChar.weapons)
					{
						...
						foreach (ArmiLicenza tin in tinte)
						{
							...
							Tnt.AddItem(tintina);
							if (Cache.Cache.MyPlayer.Character.Money >= tin.price || Cache.Cache.MyPlayer.Character.Bank >= tin.price)
								tintina.SetRightLabel("~g~" + tin.price + "$");
							else
								tintina.SetRightLabel("~r~" + tin.price + "$");
							if (Cache.Cache.MyPlayer.Character.hasWeaponTint(armi.name, Convert.ToInt32(tin.name.Substring(7)))) tintina.SetRightBadge(BadgeStyle.Ammo);
						}
						...
						Tnt.OnItemSelect += async (_menu, _item, _index) =>
						{
							...
							{
								if (Cache.Cache.MyPlayer.Character.Money >= tinte[_index].price)
								{
								...
								{
									if (Cache.Cache.MyPlayer.Character.Bank >= tinte[_index].price)
									{
