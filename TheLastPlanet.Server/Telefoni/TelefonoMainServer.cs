...
namespace TheLastPlanet.Server.Telefoni
{
	static class TelefonoMainServer
	{
		...
		public static void Init()
		{
			...
			ServerSession.Instance.AddEventHandler("lprp:onPlayerSpawn", new Action<Player>(SetupPhone));
		}
		...
		private static async void SetupPhone([FromSource] Player player)
		{
			...
			try
			{
				dynamic result = await ServerSession.Instance.Query("SELECT * FROM telefoni WHERE discord = @disc", new { disc = License.GetLicense(player, Identifier.Discord) });
				await BaseScript.Delay(0);
				...
				{
					await ServerSession.Instance.Execute("INSERT INTO telefoni (discord, playerName, phone_data) VALUES (@disc, @name, @data)", new
					{
