...
namespace TheLastPlanet.Server.Businesses
{
	internal static class PompeDiBenzinaServer
	{
		...
		private static int rentprice = ServerSession.Impostazioni.Main.RentPricePompeDiBenzina;

		public static void Init()
		{
			ServerSession.Instance.AddEventHandler("lprp:caricaStazioniGasServer", new Action(SendStationsUpdate));
			ServerSession.Instance.AddEventHandler("lprp:businesses:checkcanmanage", new Action<Player, int>(CheckCanManage));
			ServerSession.Instance.AddEventHandler("lprp:businesses:getstationcash", new Action<Player, int>(GetStationCash));
			ServerSession.Instance.AddEventHandler("lprp:businesses:changestation", new Action<Player, string, string, int, int, int, string>(ChangeStation));
			ServerSession.Instance.AddEventHandler("lprp:businesses:sellstation", new Action<Player, string, int>(SellStation));
			ServerSession.Instance.AddEventHandler("lprp:businesses:depositfuel", new Action<Player, int, int>(DepositFuel));
			ServerSession.Instance.AddEventHandler("lprp:businesses:checkfuelforstation", new Action<Player, int>(CheckFuelStation));
			ServerSession.Instance.AddEventHandler("lprp:businesses:removefuelfromstation", new Action<Player, int, int>(RemoveFuelStation));
			ServerSession.Instance.AddEventHandler("lprp:businesses:purchasestation", new Action<Player, int>(PurchaseStation));
			ServerSession.Instance.AddEventHandler("lprp:businesses:addmoneytostation", new Action<Player, int, int>(AddMoneyToStation));
			ServerSession.Instance.AddEventHandler("lprp:businesses:saddfuel", new Action<Player, int, int>(SAddFuel));
			ServerSession.Instance.AddEventHandler("lprp:businesses:saddmoney", new Action<Player, int, int>(SAddMoney));
			ServerSession.Instance.AddEventHandler("lprp:businesses:sresetmanage", new Action<Player, int>(SResetManage));
			ServerSession.Instance.AddEventHandler("lprp:businesses:addstationfunds", new Action<Player, int, int>(AddStationFunds));
			ServerSession.Instance.AddEventHandler("lprp:businesses:remstationfunds", new Action<Player, int, int>(RemStationFunds));
		}

		public static async void SendStationsUpdate()
		{
			List<StationDiBenzina> playerstations = new();
			dynamic result = await ServerSession.Instance.Query($"SELECT * FROM `businesses` WHERE businessid = {1}");
		...
		{
			dynamic result = await ServerSession.Instance.Query($"SELECT lastpaidrent, ownerchar, stationindex FROM businesses WHERE ownerchar = @charname", new { charname = p.FullName });
			await BaseScript.Delay(0);
			...
			if (result.Count > 0)
				for (int i = 0; i < result.Count; i++)
				{
					...
					{
						await ServerSession.Instance.Execute($"UPDATE `businesses` SET lastlogin = @now WHERE ownerchar = @charname AND stationindex = @index", new { now = timenow, charname = p.FullName, index = result[i].stationindex });

						if (p.Bank < rentprice)
						{
							await ServerSession.Instance.Execute($"UPDATE businesses SET identifier = '', ownerchar = '', stationname = 'Stazione di Rifornimento', thanksmessage = 'Grazie per il tuo lavoro!', cashwaiting = 0, fuelprice = 2, lastmanaged = 0, delivertype = 1, deliverallow = '', lastlogin = 0, lastpaidrent = 0 WHERE stationindex = @index", new { index = result[i].stationindex });
							BaseScript.TriggerClientEvent(ServerSession.Instance.GetPlayers[Convert.ToInt32(p.source)], "lprp:ShowNotification", "La tua stazione di rifornimento è stata ripresa per mancato pagamento.");
						}
						else
						{
							p.Bank -= rentprice;
							await ServerSession.Instance.Execute($"UPDATE businesses SET lastpaidrent = @now  WHERE stationindex = @idx", new { now = timenow, idx = result[i].stationindex });
							BaseScript.TriggerClientEvent(ServerSession.Instance.GetPlayers[Convert.ToInt32(p.source)], "lprp:ShowNotification", "{0} è stato pagato automaticamente per l'affitto della stazione.", rentprice);
						}
					}
				}
		}
		...
		{
			dynamic result = await ServerSession.Instance.Query($"SELECT `lastmanaged`, `cashwaiting`, `ownerchar` FROM `businesses` WHERE `stationindex` = @idx AND `identifier` = @ident", new { idx = sidx, ident = p.GetCurrentChar().identifiers.Discord });
			await BaseScript.Delay(0);
		...
		public static async void GetStationCash([FromSource] Player p, int sidx)
		{
			...
			dynamic result = await ServerSession.Instance.Query($"SELECT `lastmanaged`, `cashwaiting` FROM `businesses` WHERE `stationindex` = @idx AND `identifier` = @id", new { idx = sidx, id = p.GetCurrentChar().identifiers.Discord });
			await BaseScript.Delay(0);
			...
			if (result.Count > 0)
			{
				...
				{
					user.Bank += payout;
					await ServerSession.Instance.Execute($"UPDATE `businesses` SET `cashwaiting` = {0} WHERE `stationindex` = @idx AND `identifier` = @id", new { idx = sidx, id = p.GetCurrentChar().identifiers.Discord });
		...
		public static async void ChangeStation([FromSource] Player p, string stationname, string thanksmessage, int fuelCost, int Manageid, int Deltype, string Deliverylist)
		{
			...
			{
				await ServerSession.Instance.Execute($"UPDATE `businesses` SET `stationname` = @stName, `fuelprice` = @price, `thanksmessage` = @thx, `lastmanaged` = @last, `delivertype` = @deliver, `deliverallow` = @allow WHERE `stationindex` = @idx AND `identifier` = @id", new
				{
					stName = name,
					price = fuelcost,
					thx = thanks,
		...
		public static async void SellStation([FromSource] Player p, string sellname, int Manageid)
		{
			...
			if (name != null)
				foreach (Player a in ServerSession.Instance.GetPlayers.ToList())
				{
					User user = Funzioni.GetUserFromPlayerId(a.Handle);

					if (user.FullName == name)
					{
						await ServerSession.Instance.Execute($"UPDATE `businesses` SET `identifier` = @id, ownerchar = @owner WHERE stationindex = @idx", new { id = user.identifiers.Discord, owner = user.FullName, idx = manageid });
						SendStationsUpdate();
		...
		public static async void DepositFuel([FromSource] Player p, int Index, int fuelfortank)
		{
			...
			{
				dynamic result = await ServerSession.Instance.Query($"SELECT * FROM `businesses` WHERE `stationindex` = @idx", new { idx = index });
				await BaseScript.Delay(0);
					...
					if (stationfuel < stationCapacity)
					{
						...
						if (stationcash >= payout)
						{
							...
							if (allowdelivery)
							{
								...
								await ServerSession.Instance.Execute($"UPDATE `businesses` SET `fuel` = @fuel, `cashwaiting` = @cash WHERE `stationindex` = @idx", new { fuel = stationfuel, cash = newcash, idx = index });
								user.Bank += payout;
				...
				else
				{
					...
					if (stationfuel < stationCapacity)
					{
						...
						await ServerSession.Instance.Execute($"UPDATE `businesses` SET `fuel` = @fuel WHERE `stationindex` = @idx", new { fuel = stationfuel, idx = index });
		...
		public static async void CheckFuelStation([FromSource] Player p, int index)
		{
			if (index > 0)
			{
				dynamic result = await ServerSession.Instance.Query($"SELECT `fuel`, `fuelprice` FROM `businesses` WHERE `stationindex` = @idx", new { idx = index });
				await BaseScript.Delay(0);
		...
		{
			dynamic result = await ServerSession.Instance.Query($"SELECT `fuel` FROM `businesses` WHERE `stationindex` = @idx", new { idx = stationindex });
			await BaseScript.Delay(0);
			int fuel = result[0].fuel;
			fuel -= addedfuel;
			await ServerSession.Instance.Execute($"UPDATE `businesses` SET `fuel` = @fu WHERE `stationindex` = @idx", new { fu = fuel, idx = stationindex });
		}
		...
		public static async void PurchaseStation([FromSource] Player p, int sidx)
		{
			if (sidx > 0)
			{
				...
				dynamic result = await ServerSession.Instance.Query($"SELECT * FROM `businesses` WHERE `stationindex` = @idx", new { idx = sidx });
				await BaseScript.Delay(0);
				...
				else
				{
					...
					{
						await ServerSession.Instance.Execute($"UPDATE `businesses` SET `identifier`= @id, `ownerchar`= @owner, `Name`= @name, `stationname`= @stname, `lastpaidrent` = @last WHERE `stationindex`= {sidx}", new
						{
		...
		public static async void AddMoneyToStation([FromSource] Player p, int stationindex, int Amount)
		{
			...
			int oldamount = 0;
			int newamount = 0;
			dynamic result = await ServerSession.Instance.Query($"SELECT `cashwaiting` FROM `businesses` WHERE `stationindex` = @idx", new { idx = sidx });
			await BaseScript.Delay(0);
			oldamount = result[0].cashwaiting;
			newamount = oldamount + amount;
			await ServerSession.Instance.Execute($"UPDATE `businesses` SET `cashwaiting` = @cash WHERE stationindex = @idx", new { cash = newamount, idx = sidx });
		}
		...
		public static async void SAddMoney([FromSource] Player p, int closest, int Amount)
		{
			...
			if (index > 0 && Amount > -1)
			{
				dynamic result = await ServerSession.Instance.Query($"SELECT `cashwaiting` FROM `businesses` WHERE `stationindex` = @idx", new { idx = index });
				await BaseScript.Delay(0);
				int oldm = result[0].cashwaiting;
				int newm = oldm + amount;
				await ServerSession.Instance.Execute($"UPDATE `businesses` SET `cashwaiting` = @cash WHERE `stationindex` = @idx ", new { cash = newm, idx = index });
		...
		public static async void SAddFuel([FromSource] Player p, int closest, int Amount)
		{
			...
			if (index > 0 && Amount > -1)
			{
				dynamic result = await ServerSession.Instance.Query($"SELECT `fuel` FROM `businesses` WHERE `stationindex` = @idx", new { idx = index });
				await BaseScript.Delay(0);
				int oldf = result[0].fuel;
				int newf = oldf + amount;
				await ServerSession.Instance.Execute($"UPDATE `businesses` SET `fuel` = @fuel WHERE `stationindex` = @idx", new { fuel = newf, idx = index });
		...
		public static async void SResetManage([FromSource] Player p, int closest)
		{
			...
			if (index > 0)
			{
				await ServerSession.Instance.Execute($"UPDATE `businesses` SET `lastmanaged` = @last WHERE `stationindex` = @idx", new { last = DateTime.MinValue.ToString(), idx = index });
		...
		public static async void AddStationFunds([FromSource] Player p, int manageid, int amount)
		{
			...
			dynamic result = await ServerSession.Instance.Query($"SELECT `cashwaiting`, `ownerchar` FROM `businesses` WHERE `stationindex` = @idx", new { idx = manageid });
			await BaseScript.Delay(0);
				...
				if (result[0].ownerchar.ToLower() == user.FullName.ToLower())
				{
					if (money >= amount)
					{
						...
						else
						{
							...
							await ServerSession.Instance.Execute($"UPDATE `businesses` SET `cashwaiting` = @cash WHERE `stationindex` = @idx", new { cash = smoney, idx = manageid });
			...
			if (result.Count > 0)
				if (result[0].ownerchar.ToLower() == user.FullName.ToLower())
				{
					...
					if (samount >= amount)
					{
						...
						await ServerSession.Instance.Execute($"UPDATE businesses SET cashwaiting = @cash  WHERE stationindex = @idx", new { cash = samount, idx = manageid });
